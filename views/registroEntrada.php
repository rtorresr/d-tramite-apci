...
if($_SESSION['CODIGO_TRABAJADOR']!=""){
                        ...
                        <div class="card hoverable">
                            <div class="card-body">
                                <fieldset>
                                    ...
                                    <div class="row">
                                        <div class="col s4 input-field">
                                            <div class="switch">
                                                <label>
                                                    D-Tramite
                                                    <input type="checkbox" id="FlgSIGCTI" name="FlgSIGCTI" value="1">
                                    ...
                                    <div class="row">
                                        <div class="col m6 input-field">
                                            <select id="nCud" name="nCud"  class="browser-default"></select>
                                            <label for="ncud">N° CUD reingresado</label>
                                        </div>
    ...
    <script>
       ...
       $('#nCud').select2({
           ...
           "language": {
               "noResults": function(){
                   return "<p>No se encontró al cud.</p>";
               },
               "searching": function() {
                   return "Buscando..";
               },
       ...
       function validarFormulario(){
           ...
           if($('#iCodRemitente').val() === '' || $('#iCodRemitente').val() === null){
               $.alert("Falta remitente!");
               return false;
           }
       ...
       function obtenerDatosFormulario () {
           ...
           formData.append("Evento","registroMesaPartes");
           return formData;
       }
       ...
       $("#buscarConstancia").on("click", function (e) {
           e.preventDefault();
           $.ajax({
               async: false,
               url: RutaSIGTIDtramite+"ApiD-Tramite/Api/Tramite/TRA_GET_0002?id=" + $("#nroConstanciaSIGCTI").val(),
               method: "GET",
               datatype: "application/json",
               success: function (response) {
                   if (response.Success === true){
                       let data = response.EntityResult;
                       $("#nroConstanciaSIGCTIEnvio").val(data.NroConstancia);
                       $("#codInscripcionSIGCTI").val(data.CodInscripcion);

                       $('input[name="nFlgClaseDoc"]').prop('checked',true);
                       $('#datosTupa').css('display','block');

                       $('#cCodTipoDoc option[value="'+data.TipDocumento+'"]').prop('selected',true);
                       $('#cCodTipoDoc').formSelect();

                       var elemDatepicker = document.querySelector('#fechaDocumento');
                       var instance = M.Datepicker.init(elemDatepicker,{
                           i18n: {
                       ...
                       instance.setDate( new Date(data.FecDocumento) );
                       instance._finishSelection();

                       $('#cNroDocumento').val(data.NroDocumento).next().addClass('active');
                       $('#cAsunto').val(data.Asunto).next().addClass('active');
                       $('#cObservaciones').val(data.Obs).next().addClass('active');
                       $('#nNumFolio').attr("value", data.Folio);

                       $('#iCodTupaClase option[value="'+data.TupaClase+'"]').prop('selected',true);
                       $('#iCodTupaClase').formSelect();

                       CargarTUPAs(data.TupaCodigo);
                       // Agregar nombre del campo con id entidad sigcti
                       let option = new Option(data.NomEntidad,data.IdEntidad, true, true);
                       $("#iCodRemitente").append(option).trigger('change');
                       $("#cNombreRemitente").val(data.NomEntidad);

                       $("#direccionRemi").val(data.DirEntidad + "<br>" + data.Ubigeo).next().addClass('active');
                       $('#cNomRemitente').val(data.NomRpte).next().addClass('active');
                       $('#cCargoRemitente').val(data.Cargo).next().addClass('active');
                       
                       var principal = false;
                       $.each(data.ListDocumento, function (i,fila) {
                            var row = new Object();
                            row = fila;
                            row.nuevo = url_srv+fila.nuevo;
                            tblAnexos.row.add(row).draw();
                       });
                      seleccionar_todo();
                   } else {
