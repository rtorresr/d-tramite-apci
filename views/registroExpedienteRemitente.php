...
<div class="AreaTitulo">Seleccione Remitente:</div>	
    ...
    if ($color == "#e8f3ff"){
	  ...
	  if ($color == ""){
		...
		?>
    <tr bgcolor="<?=$color?>" onMouseOver="this.style.backgroundColor='#BFDEFF'; this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='<?=$color?>'" onClick="window.open('registroExpedienteSelect.php?tipoRemitente=<?=$_GET[tipoRemitente]?>&iCodRemitente=<?=$RsRem[iCodRemitente]?>&iCodTupaClase=<?=$_GET[iCodTupaClase]?>&iCodTupa=<?=$_GET['iCodTupa']?>&cCodTipoDoc=<?=(isset($_GET['cCodTipoDoc'])?$_GET['cCodTipoDoc']:'')?>&nFolios=<?=$_GET[nFolios]?>&nIndicativo=<?=$_GET[nIndicativo]?>&cObservaciones=<?=$_GET[cObservaciones]?>&nFlgDerivar=<?=$_GET[nFlgDerivar]?>', '_parent');">
