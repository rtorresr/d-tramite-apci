<?
/**************************************************************************************
NOMBRE DEL PROGRAMA: consultaInternoTrabajadoresOficina.php
SISTEMA: SISTEMA  DE TRÁMITE DOCUMENTARIO DIGITAL
OBJETIVO: Consulta de los Documentos Internos de Trabajadores por Oficina
...
<div class="AreaTitulo">Documentos Internos - Trabajadores</div>
 



							<form name="frmConsultaEntrada" method="GET" action="consultaInternoTrabajadoresOficina.php">
						<tr>
						...
						<tr>
							...
							<td width="390" align="left">
							 ...
							 &nbsp;&nbsp;&nbsp;
	                           NO<input type="checkbox" name="NO" value="1" <?php if($_GET[NO]==1) echo "checked"?> /></td>
                  ...
                  <?php // ordenamiento
                     ...
                     if($_GET['orden']==""){ $orden="DESC"; }Else{ $orden=$_GET['orden']; } ?>
							<button class="btn btn-primary" onclick="window.open('consultaInternoTrabajadoresOficina_xls.php?fDesde=<?=(isset($_GET['fDesde'])?$_GET['fDesde']:'')?>&fHasta=<?=(isset($_GET['fHasta'])?$_GET['fHasta']:'')?>&cCodificacion=<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>&SI=<?=$_GET[SI]?>&NO=<?=$_GET[NO]?>&cObservaciones=<?=$_GET[cObservaciones]?>&cAsunto=<?=(isset($_GET['cAsunto'])?$_GET['cAsunto']:'')?>&cCodTipoDoc=<?=(isset($_GET['cCodTipoDoc'])?$_GET['cCodTipoDoc']:'')?>&session=<?=$_SESSION['iCodOficinaLogin']?>&traRep=<?=$_SESSION['CODIGO_TRABAJADOR']?>&session=<?=$_SESSION['iCodOficinaLogin']?>&campo=<?=$campo?>&orden=<?=$orden?>', '_self'); return false;" onMouseOver="this.style.cursor='hand'"> <b>a Excel</b> <img src="images/icon_excel.png" width="17" height="17" border="0"> </button>
							&nbsp;
							<button class="btn btn-primary" onclick="window.open('consultaInternoTrabajadoresOficina_pdf.php?fDesde=<?=(isset($_GET['fDesde'])?$_GET['fDesde']:'')?>&fHasta=<?=(isset($_GET['fHasta'])?$_GET['fHasta']:'')?>&cCodificacion=<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>&SI=<?=$_GET[SI]?>&NO=<?=$_GET[NO]?>&cObservaciones=<?=$_GET[cObservaciones]?>&cAsunto=<?=(isset($_GET['cAsunto'])?$_GET['cAsunto']:'')?>&cCodTipoDoc=<?=(isset($_GET['cCodTipoDoc'])?$_GET['cCodTipoDoc']:'')?>&campo=<?=$campo?>&orden=<?=$orden?>', '_blank');" onMouseOver="this.style.cursor='hand'"> <b>a Pdf</b> <img src="images/icon_pdf.png" width="17" height="17" border="0"> </button>
...
if($orden=="DESC") $cambio="ASC";
    ...
    $sql="  SELECT TOP 500  Tra_M_Tramite.iCodTramite,cDescTipoDoc,cCodificacion,fFecRegistro,cAsunto,Tra_M_Tramite.cObservaciones,Tra_M_Tramite.nFlgEnvio ";
	...
	$sql.=" AND Tra_M_Trabajadores.iCodOficina='".$_SESSION['iCodOficinaLogin']."' ";
	$sql.= " ORDER BY  $campo $orden ";	   
	*/
	$sql.= " SP_CONSULTA_INTERNO_TRABAJADOR_OFICINA '$fDesde', '$fHasta',  '$_GET[SI]', '$_GET[NO]',  '%".$_GET['cCodificacion']."%', '%".$_GET['cAsunto']."%',  '%$_GET[cObservaciones]%', '".$_GET['cCodTipoDoc']."','".$_SESSION['iCodOficinaLogin']."', '$_GET['campo']', '$_GET['orden']' ";
...
<tr>
    <td width="92" class="headCellColum"><a href="<?=$_SERVER['PHP_SELF']?>?campo=Fecha&orden=<?=$cambio?>&cCodificacion=<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>&fDesde=<?=(isset($_GET['fDesde'])?$_GET['fDesde']:'')?>&fHasta=<?=(isset($_GET['fHasta'])?$_GET['fHasta']:'')?>&cCodTipoDoc=<?=(isset($_GET['cCodTipoDoc'])?$_GET['cCodTipoDoc']:'')?>&cAsunto=<?=(isset($_GET['cAsunto'])?$_GET['cAsunto']:'')?>&cReferencia=<?=$_GET[cReferencia]?>&SI=<?=$_GET[SI]?>&NO=<?=$_GET[NO]?>"  style=" text-decoration:<?php if($campo=="Fecha"){ echo "underline"; }Else{ echo "none";}?>">Fecha</a></td>
	<td width="98" class="headCellColum"><a href="<?=$_SERVER['PHP_SELF']?>?campo=DescDocumento&orden=<?=$cambio?>&cDescTipoDoc=<?=$_GET['cDescTipoDoc']?>&cCodificacion=<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>&fDesde=<?=(isset($_GET['fDesde'])?$_GET['fDesde']:'')?>&fHasta=<?=(isset($_GET['fHasta'])?$_GET['fHasta']:'')?>&cCodTipoDoc=<?=(isset($_GET['cCodTipoDoc'])?$_GET['cCodTipoDoc']:'')?>&cAsunto=<?=(isset($_GET['cAsunto'])?$_GET['cAsunto']:'')?>&cReferencia=<?=$_GET[cReferencia]?>&SI=<?=$_GET[SI]?>&NO=<?=$_GET[NO]?>"  style=" text-decoration:<?php if($campo=="DescDocumento"){ echo "underline"; }Else{ echo "none";}?>">Tipo de Documento</a></td>
	<td width="184" class="headCellColum"><a href="<?=$_SERVER['PHP_SELF']?>?campo=Asunto&orden=<?=$cambio?>&cAsunto=<?=(isset($_GET['cAsunto'])?$_GET['cAsunto']:'')?>&cCodificacion=<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>&fDesde=<?=(isset($_GET['fDesde'])?$_GET['fDesde']:'')?>&fHasta=<?=(isset($_GET['fHasta'])?$_GET['fHasta']:'')?>&cCodTipoDoc=<?=(isset($_GET['cCodTipoDoc'])?$_GET['cCodTipoDoc']:'')?>&cReferencia=<?=$_GET[cReferencia]?>&SI=<?=$_GET[SI]?>&NO=<?=$_GET[NO]?>"  style=" text-decoration:<?php if($campo=="Asunto"){ echo "underline"; }Else{ echo "none";}?>">Asunto</a></td>
	<td class="headCellColum">Observaciones</td>
    <td width="83" class="headCellColum">Enviado</td>
    <td width="83" class="headCellColum">Opciones</td>
	</tr>
<?
if($_GET['cCodificacion']=="" && $_GET['fDesde']=="" && $_GET['fHasta']=="" && $_GET['cCodTipoDoc']=="" && $_GET['cAsunto']=="" && $_GET[cReferencia]=="" && $_GET[SI]=="" && $_GET[NO]==""){	
 /* $sqlin="  SELECT Tra_M_Tramite.iCodTramite  FROM Tra_M_Tramite  LEFT OUTER JOIN Tra_M_Trabajadores ON Tra_M_Tramite.iCodTrabajadorRegistro=Tra_M_Trabajadores.iCodTrabajador  ";
    $sqlin.=" WHERE Tra_M_Tramite.nFlgTipoDoc=2 AND Tra_M_Tramite.nFlgClaseDoc=2 ";
	$sqlin.=" AND Tra_M_Trabajadores.iCodOficina='".$_SESSION['iCodOficinaLogin']."' "; */
	$sqlin= "SP_CONSULTA_INTERNO_TRABAJADOR_OFICINA_CONTEO '".$_SESSION['iCodOficinaLogin']."'";
...
?>
 ...
 <tr bgcolor="<?=$color?>" onMouseOver="this.style.backgroundColor='#BFDEFF'" OnMouseOut="this.style.backgroundColor='<?=$color?>'" >
    <td valign="middle" align="center">
    	<?
    	  echo "<div style=color:#727272>".date("d-m-Y G:i:s", strtotime(substr($Rs['fFecRegistro'], 0, -6)))/*date("d-m-Y", strtotime($Rs['fFecRegistro']))*/."</div>";
    ...
    <td valign="middle" align="left">
		...
		?>
        <br>
        <a href="registroTrabajadorDetalles.php?iCodTramite=<?=$Rs[iCodTramite]?>"  rel="lyteframe" title="Detalle del Documento" rev="width: 970px; height: 550px; scrolling: auto; border:no"><?=$Rs[cCodificacion]?></a></td>
    <td valign="middle" align="left">
    	<?
    	echo $Rs['cAsunto'];
    	?>    </td> 
    <td valign="middle" align="left"><?	echo $Rs[cObservaciones];?>    </td>
    <td valign="middle" align="center"><? 
	           	   if($Rs[nFlgEnvio]==0){
					echo "NO";
				   }
    ...
    <td valign="top">
      		...
      		if(sqlsrv_has_rows($rsDw)>0){
    			...
    			?>
		 		<a href="registroTrabajadorEdit.php?iCodTramite=<?=$Rs[iCodTramite]?>&URI=<?=$_SERVER['REQUEST_URI']?>"><i class="fas fa-edit"></i></a></td>
</tr>
...
?> 
       ...
       <?php echo paginar($pag, $total, $tampag, "consultaInternoTrabajadoresOficina.php?cCodificacion=".(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')."&fDesde=".(isset($_GET['fDesde'])?$_GET['fDesde']:'')."&fHasta=".(isset($_GET['fHasta'])?$_GET['fHasta']:'')."&cCodTipoDoc=".(isset($_GET['cCodTipoDoc'])?$_GET['cCodTipoDoc']:'')."&cAsunto=".(isset($_GET['cAsunto'])?$_GET['cAsunto']:'')."&cReferencia=".$_GET[cReferencia]."&SI=".$_GET[SI]."&NO=".$_GET[NO]."&pag=");?>
