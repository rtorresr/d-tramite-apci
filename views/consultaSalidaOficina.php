...
<div class="AreaTitulo">Consulta >> Doc. de salida oficina</div>
	...
	?>                        
						...
						<tr>
          				...
          				sqlsrv_free_stmt($rsTipo);
							...
							<td colspan="4" align="left"><input type="txt" name="cAsunto" value="<?=(isset($_GET['cAsunto'])?$_GET['cAsunto']:'')?>" size="65" class="FormPropertReg form-control">							</td>
						</tr>
						<tr>
							<td width="120" >Instituci&oacute;n:</td>
							<td width="390" colspan="4" align="left"><input type="txt" name="cNombre" value="<?=(isset($_GET['cNombre'])?$_GET['cNombre']:'')?>" size="55" class="FormPropertReg form-control"></td>
               ...
               <td >Registros :</td>
              <td  > Propios:							</td>
							<td  align="left"><input type="checkbox" name="RegistroPersonal" value="1" <?php if($_GET[RegistroPersonal]==1) echo "checked"?> /></td>
							<td  >Solicitados:</td>
							<td  align="left"><input type="checkbox" name="RegistroSolicitado" value="1" <?php if($_GET[RegistroSolicitado]==1) echo "checked"?> /></td>
						</tr>
									
					<tr>
						  <td width="120" >Respuesta:</td>
              <td colspan="4" align="left">
              		<select name="Respuesta" id="Respuesta" class="FormPropertReg form-control" <?php if($_GET[RespuestaSI]!=1){ echo "disabled";} ?> >
             ...
             <tr>           
              ...
              <td align="left">
                  ...
                  | &nbsp; &nbsp; M&uacute;ltiple &nbsp; <img src="images/page_copy.png" width="16" height="16" border="0">&nbsp;&nbsp;&nbsp;&nbsp;
                  | &nbsp; &nbsp; Copia a Oficina&nbsp; <img src="images/icon_delegar.png" width="16" height="16" border="0">                           </td>
               </tr>
              </table>
							...
							�
             <button class="btn btn-primary" onclick="window.open('<?=$_SERVER['PHP_SELF']?>', '_self'); return false;" onMouseOver="this.style.cursor='hand'">
              ...
              ?>
							<button class="btn btn-primary" onclick="window.open('consultaSalidaOficina_xls.php?fecini=<?=$fecini?>&fecfin=<?=$fecfin?>&RespuestaSI=<?=$_GET[RespuestaSI]?>&RespuestaNO=<?=$_GET[RespuestaNO]?>&cCodificacion=<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>&cAsunto=<?=(isset($_GET['cAsunto'])?$_GET['cAsunto']:'')?>&cObservaciones=<?=$_GET[cObservaciones]?>&cCodTipoDoc=<?=(isset($_GET['cCodTipoDoc'])?$_GET['cCodTipoDoc']:'')?>&cNombre=<?=(isset($_GET['cNombre'])?$_GET['cNombre']:'')?>&Respuesta=<?=$_GET[Respuesta]?>&RegistroPersonal=<?=$_GET[RegistroPersonal]?>&RegistroSolicitado=<?=$_GET[RegistroSolicitado]?>&campo=<?=$campo?>&orden=<?=$orden?>&traRep=<?=$_SESSION['CODIGO_TRABAJADOR']?>&session=<?=$_SESSION['iCodOficinaLogin']?>', '_self'); return false;" onMouseOver="this.style.cursor='hand'"> <b>a Excel</b> <img src="images/icon_excel.png" width="17" height="17" border="0"> </button>
							�
                           <button class="btn btn-primary" onclick="window.open('consultaSalidaOficina_pdf.php?fecini=<?=$fecini?>&fecfin=<?=$fecfin?>&RespuestaSI=<?=$_GET[RespuestaSI]?>&RespuestaNO=<?=$_GET[RespuestaNO]?>&cCodificacion=<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>&cAsunto=<?=(isset($_GET['cAsunto'])?$_GET['cAsunto']:'')?>&cObservaciones=<?=$_GET[cObservaciones]?>&cCodTipoDoc=<?=(isset($_GET['cCodTipoDoc'])?$_GET['cCodTipoDoc']:'')?>&cNombre=<?=(isset($_GET['cNombre'])?$_GET['cNombre']:'')?>&Respuesta=<?=$_GET[Respuesta]?>&RegistroPersonal=<?=$_GET[RegistroPersonal]?>&RegistroSolicitado=<?=$_GET[RegistroSolicitado]?>&campo=<?=$campo?>&orden=<?=$orden?>', '_blank');" onMouseOver="this.style.cursor='hand'"> <b>a Pdf</b> <img src="images/icon_pdf.png" width="17" height="17" border="0"> </button>    			</td>
					</tr>
					</form>
					</table>
			</fieldset>
...
if($_GET['cCodificacion']=="" && $_GET['cCodTipoDoc']=="" && $_GET['cNombre']=="" && $_GET[RespuestaSI]=="" && $_GET[RespuestaNO]=="" && $_GET[Respuesta]=="" && $_GET['fDesde']==""  && $_GET['fHasta']=="" &&
 $_GET['cAsunto']=="" && $_GET[cObservaciones]=="" && $_GET[RegistroPersonal]=="" && $_GET[RegistroSolicitado]=="" ){
	
	    $sqlCo="SP_CONSULTA_SALIDA_OFICINA_CONTEO '$fecini', '$fecfin','$_GET[RespuestaSI]', '$_GET[RespuestaNO]', '%".$_GET['cCodificacion']."%', '%".$_GET['cAsunto']."%', '%$_GET[cObservaciones]%', '".$_GET['cCodTipoDoc']."','%$_GET['cNombre']%','$_GET[Respuesta]', '$_GET[RegistroPersonal]', '$_GET[RegistroSolicitado]','".$_SESSION['iCodOficinaLogin']."' ";
	   $rsCo=sqlsrv_query($cnx,$sqlCo);
	   
	   $RsCo=sqlsrv_fetch_array($rsCo);
  $totalCo =   $RsCo[0];
//	echo "entro";
	}

    $sql = "SP_CONSULTA_SALIDA_OFICINA '$fecini', '$fecfin','$_GET[RespuestaSI]', '$_GET[RespuestaNO]', '%".$_GET['cCodificacion']."%', '%".$_GET['cAsunto']."%', '%$_GET[cObservaciones]%', '".$_GET['cCodTipoDoc']."','%$_GET['cNombre']%','$_GET[Respuesta]', '$_GET[RegistroPersonal]', '$_GET[RegistroSolicitado]','".$_SESSION['iCodOficinaLogin']."','$campo','$orden' ";
...
if($numrows==0){ 
	...
	if($totalCo !="")
	{  echo "TOTAL DE REGISTROS : ".$totalCo;}
	else {  echo "TOTAL DE REGISTROS : ".$numrows;}
...
?>
 ...
 <tr bgcolor="<?=$color?>" onMouseOver="this.style.backgroundColor='#BFDEFF'" OnMouseOut="this.style.backgroundColor='<?=$color?>'" >
    ...
    if($Rs[nFlgClaseDoc]==1){
    	echo "<a style=\"color:#0067CE\" href=\"registroSalidaDetalles.php?iCodTramite=".$Rs[iCodTramite]."\" rel=\"lyteframe\" title=\"Detalle del TRÁMITE\" rev=\"width: 970px; height: 350px; scrolling: auto; border:no\">";
    }
	...
	$sqlCorre="SELECT * FROM Tra_M_Correlativo_Salida WHERE cCodTipoDoc='$Rs[cCodTipoDoc]' AND iCodOficina='$Rs[iCodOficinaRegistro]' AND nNumAno='$AnhoRegistro'";
    ...
    <td valign="middle">
			...
			if ($RsTramitePDF->documentoElectronico != NULL and $RsTramitePDF->documentoElectronico!=' ') {
            ?>
            <a href="registroInternoDocumento_pdf.php?iCodTramite=<?php echo $RsTramitePDF->iCodTramite;?>" target="_blank" title="Documento Electrónico"><img src="images/1471041812_pdf.png" border="0" height="17" width="17"></a>
            <?php } ?>
			...
			echo " <a style=\"color:#0067CE\" href=\"registroRespuestaOficina.php?iCodTramite=".$Rs[iCodTramite]."\" rel=\"lyteframe\" title=\"Detalle de la Observacion\" rev=\"width: 410px; height: 280px; scrolling: no; border:no\"><img src='images/icon_avance.png' width='16' height='16' border='0'></a>";
   
			if($RsCorre[nCorrelativo]==$Correlativo){
				/*	$sqlRep="SELECT fFecRecepcion FROM Tra_M_Tramite_Movimientos WHERE iCodTramite=$Rs[iCodTramite] ";
					$rsRep=sqlsrv_query($cnx,$sqlRep);
					$sqlRep2="SELECT fFecRecepcion FROM Tra_M_Tramite_Movimientos WHERE iCodTramite=$Rs[iCodTramite]  AND (fFecRecepcion IS NULL or fFecRecepcion='')";
					$rsRep2=sqlsrv_query($cnx,$sqlRep2);
					if(sqlsrv_has_rows($rsRep)==sqlsrv_has_rows($rsRep2)){*/
				echo "&nbsp;<a href=\"registroData.php?iCodTramite=".$Rs[iCodTramite]."&URI=".$_SERVER['REQUEST_URI']."&iCodCorrelativo=".$RsCorre[iCodCorrelativo]."&nCorrelativo=".($Correlativo-1)."&opcion=28\" onClick=\"return ConfirmarBorrado();\"><img src=\"images/icon_del.png\" width=\"16\" height=\"16\" alt=\"Eliminar Documento\" border=\"0\"></a>";
			}
			//	} 
	?> 
		   </td>
