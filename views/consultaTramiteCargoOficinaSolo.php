...
<div class="AreaTitulo">Consulta >> Control de Cargos (L)</div>
	...
	?>                             
		                ...
		                ?>
							...
							<td colspan="5" align="left" class="CellFormRegOnly">
								<input type="txt" name="cDireccion" value="<?=$_REQUEST[cDireccion]?>" size="57" class="FormPropertReg form-control">
							</td>
						</tr>
...
*/ 
 /*  $sql=" SELECT TOP 500 iCodAuto,cOrdenServicio,cSiglaOficina,cDescTipoDoc,iCodTrabajadorEnvio,cNombresTrabajador,cApellidosTrabajador,fRespuesta,fEntrega,cFlgEnvio,cFlgEstado,cRecibido,cNomOficina,
  ...
  $sql.= " SP_CONSULTA_TRAMITE_CARGO_SOLO_OFICINA '$fecini', '$fecfin','$_REQUEST[ChxfRespuesta]','$_REQUEST[fEntrega]', '%$_REQUEST[cCodificacion]%', '%$_REQUEST['cNombre']%','%$_REQUEST[cDireccion]%', '$_REQUEST[cCodTipoDoc]', '$_REQUEST[cOrdenServicio]', '$_REQUEST[cFlgUrgente]', '$_REQUEST[iCodTrabajadorEnvio]', '$_REQUEST[cFlgLocal]','$_REQUEST[cFlgNacional]','$_REQUEST[cFlgInternacional]','$_REQUEST['iCodOficina']','$_REQUEST[cFlgEstado]','$_REQUEST[cCodDepartamento]','$_REQUEST[cCodProvincia]','$_REQUEST[cCodDistrito]','$campo','$orden','".$_SESSION['iCodOficinaLogin']."'";
  // echo $sql; 
...
<tr>
  ...
  <td width="143" class="headCellColum">
  	<a href="<?=$_SERVER['PHP_SELF']?>?campo=Estado&orden=<?=$cambio?>&cFlgEstado=<?=$_GET[cFlgEstado]?>&fDesde=<?=(isset($_GET['fDesde'])?$_GET['fDesde']:'')?>&fHasta=<?=(isset($_GET['fHasta'])?$_GET['fHasta']:'')?>&cCodificacion=<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>&iCodOficina=<?=$_GET['iCodOficina']?>&cCodTipoDoc=<?=(isset($_GET['cCodTipoDoc'])?$_GET['cCodTipoDoc']:'')?>&cFlgUrgente=<?=$_GET[cFlgUrgente]?>&cFlgLocal=<?=$_GET[cFlgLocal]?>&cFlgNacional=<?=$_GET[cFlgNacional]?>&iCodTrabajadorEnvio=<?=$_GET[iCodTrabajadorEnvio]?>&cNombre=<?=(isset($_GET['cNombre'])?$_GET['cNombre']:'')?>&cDireccion=<?=$_GET[cDireccion]?>&cOrdenServicio=<?=$_GET[cOrdenServicio]?>&ChxfRespuesta=<?=$_GET[ChxfRespuesta]?>"  style=" text-decoration:<?php if($campo=="Estado"){ echo "underline"; }Else{ echo "none";}?>">Estado Cargo</a>
  </td>

  <td width="146" class="headCellColum">
  	<a href="<?=$_SERVER['PHP_SELF']?>?campo=Estado&orden=<?=$cambio?>&cFlgEstado=<?=$_GET[cFlgEstado]?>&fDesde=<?=(isset($_GET['fDesde'])?$_GET['fDesde']:'')?>&fHasta=<?=(isset($_GET['fHasta'])?$_GET['fHasta']:'')?>&cCodificacion=<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>&iCodOficina=<?=$_GET['iCodOficina']?>&cCodTipoDoc=<?=(isset($_GET['cCodTipoDoc'])?$_GET['cCodTipoDoc']:'')?>&cFlgUrgente=<?=$_GET[cFlgUrgente]?>&cFlgLocal=<?=$_GET[cFlgLocal]?>&cFlgNacional=<?=$_GET[cFlgNacional]?>&iCodTrabajadorEnvio=<?=$_GET[iCodTrabajadorEnvio]?>&cNombre=<?=(isset($_GET['cNombre'])?$_GET['cNombre']:'')?>&cDireccion=<?=$_GET[cDireccion]?>&cOrdenServicio=<?=$_GET[cOrdenServicio]?>&ChxfRespuesta=<?=$_GET[ChxfRespuesta]?>"  style=" text-decoration:<?php if($campo=="Estado"){ echo "underline"; }Else{ echo "none";}?>"></a>
  </td>
