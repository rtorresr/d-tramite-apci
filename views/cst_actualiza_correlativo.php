...
/* * ************************************************************************************
  NOMBRE DEL PROGRAMA: ad_actualiza_corre_interno.php
  SISTEMA: SISTEMA  DE TRÁMITE DOCUMENTARIO DIGITAL
  OBJETIVO: Procesamiento de Información de la Tabla Maestra de Correlativos
...
switch ($_POST[opcion]) {
    case 1:
        For ($j = 0; $j < count($_POST[cCorrelativo]); $j++) {
        ...
        header("Location: ../views/cst_correlativo_interno.php?iCodOficina=" . $_REQUEST['iCodOficina'] . "&anho=" . $_REQUEST[anho]);
        break;
    case 2:
        ...
        $rsU = sqlsrv_query($cnx,$sqlU, $cnx);
        header("Location: ../views/cst_correlativo_interno.php?iCodOficina=" . $_REQUEST['iCodOficina']);
        break;
    case 3:
        For ($j = 0; $j < count($_POST[cCorrelativo]); $j++) {
            $cCorrelativo = $_POST[cCorrelativo];
        ...
        header("Location: ../views/cst_correlativo_salida.php?iCodOficina=" . $_REQUEST['iCodOficina'] . "&anho=" . $_REQUEST[anho]);
        break;
    case 4:
        $sqlU = " INSERT INTO Tra_M_Correlativo_Salida(cCodTipoDoc,iCodOficina,nNumAno,nCorrelativo) ";
        $sqlU.= " VALUES ( $_POST[cCodTipoDoc],$_REQUEST['iCodOficina'], $_POST[nNumAno], 0) ";
        $rsU = sqlsrv_query($cnx,$sqlU, $cnx);
        header("Location: ../views/cst_correlativo_salida.php?iCodOficina=" . $_REQUEST['iCodOficina']);
        break;
    case 5:
        For ($j = 0; $j < count($_POST[cCorrelativo]); $j++) {
            $cCorrelativo = $_POST[cCorrelativo];
