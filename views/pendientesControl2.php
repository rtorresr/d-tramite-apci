...
<div class="AreaTitulo">documentos pendientes</div>	
						...
						<tr>
							...
							<td width="390" align="left"><input type="checkbox" name="Entrada" value="1" <?php if($_GET['Entrada']==1) echo "checked"?> onclick="activaEntrada();" />
							  Entrada  &nbsp;&nbsp;&nbsp;
						  <input type="checkbox" name="Interno" value="1" <?php if($_GET['Interno']==1) echo "checked"?> onclick="activaInterno();">Internos</td>
    ...
    $sqlTra.="WHERE Tra_M_Tramite.iCodTramite=Tra_M_Tramite_Movimientos.iCodTramite ";
	   ...
	   ?>	
				...
				<tr>
                <td class="headColumnas"><a href="<?=$_SERVER['PHP_SELF']?>?campo=Tra_M_Tramite.cCodificacion&orden=<?=$cambio?>&Tra_M_Tramite.cCodificacion=<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>"  style=" text-decoration:<?php if($campo=="Tra_M_Tramite.cCodificacion"){ echo "underline"; }Else{ echo "none";}?>">N&ordm; Tramite</a></td>
				<td class="headColumnas"><a href="<?=$_SERVER['PHP_SELF']?>?campo=Tra_M_Tramite.nFlgTipoDoc&orden=<?=$cambio?>&nFlgTipoDoc=<?=(isset($_GET['nFlgTipoDoc'])?$_GET['nFlgTipoDoc']:'')?>"  style=" text-decoration:<?php if($campo=="Tra_M_Tramite.nFlgTipoDoc"){ echo "underline"; }Else{ echo "none";}?>">Tipo de Documento</a></td>
				...
				<td class="headColumnas"><a href="<?=$_SERVER['PHP_SELF']?>?campo=Tra_M_Tramite.cAsunto&orden=<?=$cambio?>&cAsunto=<?=(isset($_GET['cAsunto'])?$_GET['cAsunto']:'')?>"  style=" text-decoration:<?php if($campo=="Tra_M_Tramite.cAsunto"){ echo "underline"; }Else{ echo "none";}?>">Asunto / Procedimiento TUPA</a></td>
				<td class="headColumnas">Derivado</td>
				<td class="headColumnas"><a href="<?=$_SERVER['PHP_SELF']?>?campo=Tra_M_Tramite.fFecRecepcion&orden=<?=$cambio?>&fFecRecepcion=<?=(isset($_GET['fFecRecepcion'])?$_GET['fFecRecepcion']:'')?>"  style=" text-decoration:<?php if($campo=="Tra_M_Tramite.fFecRecepcion"){ echo "underline"; }Else{ echo "none";}?>">Recepción</a></td>
				<td class="headColumnas"><a href="<?=$_SERVER['PHP_SELF']?>?campo=Tra_M_Tramite.iCodTrabajadorDerivar&orden=<?=$cambio?>&iCodTrabajadorDerivar=<?=(isset($_GET['iCodTrabajadorDerivar'])?$_GET['iCodTrabajadorDerivar']:'')?>"  style=" text-decoration:<?php if($campo=="Tra_M_Tramite.iCodTrabajadorDerivar"){ echo "underline"; }Else{ echo "none";}?>">Responsable/ Delegado</a></td>
				<td class="headColumnas"><a href="<?=$_SERVER['PHP_SELF']?>?campo=Tra_M_Tramite.cCodificacion&orden=<?=$cambio?>&cCodificacion=<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>"  style=" text-decoration:<?php if($campo=="Tra_M_Tramite.cCodificacion"){ echo "underline"; }Else{ echo "none";}?>">Estado</a></td>
        ...
        <td width="110" valign="middle">
        	<?php if($RsTra['fFecRecepcion']==""){?>
 					...
 					<?php } else{?>
              		...
              		<a href="registroDiasVigencia.php?iCodTramite=<?=$RsTra['iCodTramite']?>"  rel="lyteframe" title="Detalle del TRÁMITE" rev="width: 400px; height: 160px; scrolling: auto; border:no"><img src="images/icon_calendar.png" width="22" height="20" border="0"></a>
							<?php}?>
