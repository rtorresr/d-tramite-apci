...
//  Sql es ejecutado en SP
  ...
  case 4: //registro interno trabajador
	...
	$RsTJefe=sqlsrv_fetch_array($rsTJefe);
		...
		$sqlMov="INSERT INTO Tra_M_Tramite_Movimientos ";
			$sqlMov.="(		iCodTramite,			iCodTrabajadorRegistro,	  nFlgTipoDoc, 				 iCodOficinaOrigen,           iCodOficinaDerivar,			iCodTrabajadorDerivar,	 iCodTrabajadorDelegado,	fFecDelegado,  fFecMovimiento, cObservacionesDelegado,    
 cAsuntoDerivar, 	cObservacionesDerivar , 	fFecDerivar , 	 nEstadoMovimiento,  cFlgTipoMovimiento)";
			$sqlMov.=" VALUES ";
...
$sqlTJefe=" SELECT iCodTrabajador FROM Tra_M_Trabajadores WHERE iCodOficina = '$_POST[iCodOficinaMov]' and nFlgEstado =1 and iCodCategoria =5 ";
    ...
    $sqlUpd.="WHERE iCodTramite='$_POST[iCodTramite]'";
		...
		for ($i=0;$i<count($_POST[lstTrabajadoresSel]);$i++){
			$lstTrabajadoresSel=$_POST[lstTrabajadoresSel];
			$sqlMov="INSERT INTO Tra_M_Tramite_Movimientos ";
			$sqlMov.="(iCodTramite,		iCodTrabajadorRegistro,				nFlgTipoDoc,   iCodOficinaOrigen,   iCodOficinaDerivar,   iCodTrabajadorDerivar,   iCodTrabajadorEnviar,    fFecMovimiento,           nEstadoMovimiento,  cAsuntoDerivar ,                cObservacionesDerivar, 		 fFecDerivar,              cFlgTipoMovimiento)";
			$sqlMov.=" VALUES ";
			$sqlMov.="('$_POST[iCodTramite]', '".$_SESSION['CODIGO_TRABAJADOR']."', '2', 	  '".$_SESSION['iCodOficinaLogin']."', '".$_SESSION['iCodOficinaLogin']."',  '$lstTrabajadoresSel[$i]', '$lstTrabajadoresSel[$i]', '$_POST[fFecMovimiento]', 1,	 '$_POST['cAsunto']', '$_POST[cObservaciones]', '$fFecActual', 2)";
	   		$rsMov=sqlsrv_query($cnx,$sqlMov);
		}
...
iCodTramiteDerivar)";
  ...
  case 24: //a�adir movimiento de oficina edit
			...
			echo "<input type=hidden name=pag value=\"".$_POST[pag]."\">";
   			...
   			echo "</html>";
	break;
  case 25: //a�adir movimiento temporal
		for ($i=0;$i<count($_POST[lstOficinasSel]);$i++){
			$lstOficinasSel=$_POST[lstOficinasSel];
      ...
      $RsTrb=sqlsrv_fetch_array($rsTrb);
    	...
    	sqlsrv_free_stmt($rsTrb);
            ...
            echo "<input type=hidden name=iCodMovimientoAccion value=\"".$_POST[iCodMovimientoAccion2]."\">";
   			...
   			echo "</html>";
	break;
	case 26: //a�adir movimiento temporal
		for ($i=0;$i<count($_POST[lstOficinasSel]);$i++){
			$lstOficinasSel=$_POST[lstOficinasSel];
