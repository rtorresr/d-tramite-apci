...
if($orden=="DESC") $cambio="ASC";
   ...
   if( $fecfinx!=''){
	...
	$sql.= " SP_CONSULTA_INTERNO_OFICINA '$fecinix', '$fecfinx',  '$SIx', '$NOx',  '%$cCodificacionx%', '%$cAsuntox%',  '%$cObservacionesx%', '$cCodTipoDocx' ,'$iCodOficinax','$OfiLog', '$campox', '$ordenx' ";
...
if($_GET['cCodificacion']=="" && $fecinix=="" && $fecfinx=="" && $_GET['cCodTipoDoc']=="" && $_GET['cAsunto']=="" &&  $_GET['iCodOficina']=="" && $_GET[SI]=="" && $_GET[NO]=="" ){
   $sqlin= " SP_CONSULTA_INTERNO_OFICINA_LISTA '$OfiLog' ";
...
?>
    ...
    <td valign="top" align="left">
      <?php
			...
			<br>
			<?php 
				if ($Rs[nFlgTipoDerivo] == 1) {
					echo "<font color=grey>(Tipo Derivado)</font>";
				}else{
					if($Rs[nFlgEnvio]==0){
    ...
    <!-- <td valign="top" align="left">
		<?php 
			...
			// 					 WHERE iCodTrabajador='".$_SESSION['JEFE']."'";
			$sqlTra = "SELECT cNombresTrabajador, cApellidosTrabajador FROM Tra_M_Trabajadores 
								 WHERE iCodTrabajador = 
								 	(SELECT iCodTrabajador FROM Tra_M_Perfil_Ususario 
								 	 WHERE iCodPerfil = 3 AND iCodOficina = 
								 	 	(SELECT iCodOficina FROM Tra_M_Trabajadores WHERE iCodTrabajador = $_SESSION['CODIGO_TRABAJADOR']))";
    ...
    <td valign="top">
    	...
    	//echo $sqlRep."<br>";
			...
			$sqlDev = "SELECT iCodTramiteDerivar FROM Tra_M_Tramite_Movimientos 
								 WHERE iCodTramiteDerivar = $Rs[iCodTramite] AND cFlgTipoMovimiento = 1 ";
