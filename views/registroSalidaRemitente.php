...
<div class="AreaTitulo">Seleccione Remitente:</div>	
    ...
    if ($color == "#e8f3ff"){
	  ...
	  if ($color == ""){
		...
		?>
    <tr bgcolor="<?=$color?>" onMouseOver="this.style.backgroundColor='#BFDEFF'; this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='<?=$color?>'" onClick="window.open('registroSalidaSelect.php?tipoRemitente=<?=$_GET[tipoRemitente]?>&iCodRemitente=<?=$RsRem[iCodRemitente]?>&cCodTipoDoc=<?=(isset($_GET['cCodTipoDoc'])?$_GET['cCodTipoDoc']:'')?>&fFecDocumento=<?=$_GET['fFecDocumento']?>&iCodTrabajadorSolicitado=<?=$_GET[iCodTrabajadorSolicitado]?>&iCodOficinaSolicitado=<?=$_GET[iCodOficinaSolicitado]?>&cReferencia=<?=$_GET[cReferencia]?>&cAsunto=<?=(isset($_GET['cAsunto'])?$_GET['cAsunto']:'')?>&cObservaciones=<?=$_GET[cObservaciones]?>&iCodOficinaResponsable=<?=$_GET[iCodOficinaResponsable]?>&iCodIndicacion=<?=$_GET[iCodIndicacion]?>&nFlgRpta=<?=$_GET[nFlgRpta]?>&nNumFolio=<?=$_GET[nNumFolio]?>&fFecPlazo=<?=$_GET[fFecPlazo]?>&cSiglaAutor=<?=$_GET[cSiglaAutor]?>&nFlgEnvio=<?=$_GET[nFlgEnvio]?>&nFlgClaseDoc=<?=$_GET[nFlgClaseDoc]?>', '_parent');">
