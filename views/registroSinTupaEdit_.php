...
<div class="AreaTitulo">Actualizacion - Registro de entrada sin tupa</div>
		...
		<td class="FondoFormRegistro">
          ...
          sqlsrv_free_stmt($rsTipo);
			...
			<td valign="top" colspan="3"><input type="text" style="text-transform:uppercase" name="cNroDocumento" value="<?php if($_GET[clear]==""){ echo trim($Rs['cNroDocumento']); }Else{ echo $_POST['cNroDocumento'];}?>" class="FormPropertReg form-control" style="width:250px" />&nbsp;<span class="FormCellRequisito">*</span></td>
			</tr>
          ...
          $RsRmt=sqlsrv_fetch_array($rsRmt);
			...
			<td valign="top" colspan="3"><input type="text" name="cReferencia" style="text-transform:uppercase" value="<?php if($_GET[clear]==""){ echo trim($Rs[cReferencia]); }Else{ echo $_POST[cReferencia];}?>" class="FormPropertReg form-control" style="width:250px" /></td>
			</tr>
			...
			<td>
							<?
							$sqlMov="SELECT TOP 1 * FROM Tra_M_Tramite_Movimientos WHERE iCodTramite='$_GET[iCodTramite]' and cFlgTipoMovimiento!=4 ORDER BY iCodMovimiento ASC";
              ...
              if(sqlsrv_has_rows($rsMov)>0){
              	...
              	echo "<input type=hidden name=iCodTra value=".$RsMov[iCodTrabajadorDerivar].">";
							...
							<option value="">Seleccione:</option>
							<?
							$sqlOfic="SP_OFICINA_LISTA_COMBO";
              $rsOfic=sqlsrv_query($cnx,$sqlOfic);
              while ($RsOfic=sqlsrv_fetch_array($rsOfic)){
              	if($_GET[clear]==""){
          					if($RsOfic["iCodOficina"]==$iCodOficinaResponsable){
          						$selecOfi="selected";
          					}Else{
          						$selecOfi="";
          					}
			...
			<td>
              ...
              while ($RsTrb=sqlsrv_fetch_array($rsTrb)){
              	if($_GET[clear]==""){
              			if($RsTrb[iCodTrabajador]==$iCodTrabajadorResponsable){
              				$selecTrab="selected";
              			}Else{
              				$selecTrab="";
              			}
			...
			<tr>
			<td valign="top"  width="200">Indicación:</td>
			<td valign="top">
			...
			<tr>
          	...
          	if(sqlsrv_has_rows($rsDig)>0){
				...
				<td valign="top" class="CellFormRegOnly"><input type="text" name="nTiempoRespuesta" value="<?php if($_GET[clear]==""){ echo trim($Rs[nTiempoRespuesta]); }Else{ echo $_POST[nTiempoRespuesta];}?>" class="FormPropertReg form-control" style="width:40px;text-align:right"  maxlength="3" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;"/> d�as</td>
			</tr>
