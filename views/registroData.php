...
date_default_timezone_set('America/Lima');
  ...
  switch ($_POST['opcion']??'') {
    ...
    case 1:
  		...
  		$rsCorr = sqlsrv_query($cnx,"SELECT Max(nCorrelativo) as nCorrelativo FROM Tra_M_Correlativo  WHERE nFlgTipoDoc = 1 AND nNumAno=nNumAno=year(GETDATE()) ");
  		...
  		$rsUpdCorr = sqlsrv_query($cnx,"UPDATE Tra_M_Correlativo SET nCorrelativo=nCorrelativo+1 WHERE nFlgTipoDoc = 1 AND nNumAno=nNumAno=year(GETDATE()) ");
    	...
    	/* I MAX */
    	// $clave= rand(1000000000,9999999999);
    	...
    	if ($_POST['nFlgClaseDoc'] == 1){ //sql con tupa
            ...
            // 	$_POST[nFlgEnvio] = "";
            // }
            if (isset($_POST['ActivarDestino'])) {
                $mantenerPendiente = 1; // SI ,HA SIDO ENVIADO CON TUPA y esta completo
            }else{
                $mantenerPendiente = 0; // NO, HA SIDO ENVIADO CON TUPA y no esta completo
            }

            /* I cambio posicion de fecha  MAX ( se deberia sumar el plazo a la fecha para obtener la fecha maxima)*/
        ...
        if(isset($_POST['iCodOficinaResponsable'])){
			//  Sql es ejecutado en
		...
		if($_FILES['fileUpLoadDigital']['name']!==''){
  			...
  			if($extension[$num]==='exe' || $extension[$num]==='dll' || $extension[$num]==='EXE' || $extension[$num]==='DLL'){
  			    $nFlgRestricUp=1;
   			}else{
                ...
                $sqlDigt="INSERT INTO Tra_M_Tramite_Digitales (iCodTramite, cNombreOriginal, cNombreNuevo, iCodTipoDigital) VALUES ('".$RsUltTra['iCodTramite']."', '".$cNombreOriginal."', '".$nuevo_nombre."',2)";
	...
	//--- REGISTRAR INTERNO OFICINAS ----/////////////////////////////////////////////////////////////
    case 2:
		$esProyecto = $_POST['proyecto']??'';

		if($esProyecto === 'Si'){

            $sqlProfesional = "SELECT iCodPerfil FROM Tra_M_Perfil_Ususario WHERE iCodTrabajador = '".$_SESSION['CODIGO_TRABAJADOR']."' AND iCodOficina = '".$_SESSION['iCodOficinaLogin']."'";
            ...
            $sqlCorr = "SELECT * FROM Tra_M_Correlativo_Proyecto_Oficina WHERE cCodTipoDoc='".$_POST['cCodTipoDoc']."' AND iCodOficina='".$_SESSION['iCodOficinaLogin']."' AND nNumAno='".$nNumAno."'";
            ...
            if (sqlsrv_has_rows($rsCorr) > 0){
                ...
                $sqlUpd = "UPDATE Tra_M_Correlativo_Proyecto_Oficina  SET nCorrelativo='".$nCorrelativo."' WHERE iCodCorrelativo='".$RsCorr['iCodCorrelativo']."'";
                $rsUpd  = sqlsrv_query($cnx,$sqlUpd);
            }else{
                $sqlAdCorr = "INSERT INTO Tra_M_Correlativo_Proyecto_Oficina (cCodTipoDoc, iCodOficina, nNumAno, nCorrelativo) VALUES ('".$_POST['cCodTipoDoc']."','".$_SESSION['iCodOficinaLogin']."', '".$nNumAno."',1)";
            ...
            // armar correlativo
            // ORIGINAL
            $cCodificacion = add_ceros($nCorrelativo,3).'-'.date('Y').'/'.trim($RsSigla['cSiglaOficina']);
            ...
            //$rsUpdCorrI = sqlsrv_query($cnx,"UPDATE Tra_M_Correlativo SET nCorrelativo=".$CorrelativoAsignarI."  WHERE nFlgTipoDoc='9' AND nNumAno='".$nNumAno."'");

            //$year = substr(date('Y'), -2);
            //$cCodificacionI = 'I'.$year.add_ceros($CorrelativoAsignarI,5);
            ...
            $iCodTramiteRespuesta = $_POST['iCodTramiteRespuesta']??0;

            $sqlAdd =" SP_DOC_ENTRADA_INTERNO_PROYECTO_INSERT '".$cCodificacion."',	'".$_SESSION['CODIGO_TRABAJADOR']."', '".$_SESSION['iCodOficinaLogin']."','".$_POST['cCodTipoDoc']."', '".$fFecActual."', '".$RsJefe['iCodTrabajador']."', '".$_POST['cReferencia']."', '".$_POST['cAsunto']."', '".$_POST['cObservaciones']."', '".$_POST['nFlgRpta']."', '".$_POST['nNumFolio']."', ".$fFecPlazo.",0, '".$fFecActual."','".$_POST['cSiglaAutor']."','','".$_POST['archivoFisico']."','".$_POST['editorOficina']."','001-2018'";
            $rs = sqlsrv_query($cnx,$sqlAdd);



            ///////////////////
            $sqlCodTramite = "SELECT TOP 1 iCodProyecto FROM Tra_M_Proyecto ORDER BY iCodProyecto DESC";
            $rsCodTramite  = sqlsrv_query($cnx,$sqlCodTramite);
            $RsCodTramite  = sqlsrv_fetch_array($rsCodTramite);
            $nuevoCodigoTramite = $RsCodTramite['iCodProyecto'];

            //$sqlUpdate = "UPDATE Tra_M_Tramite SET cCodificacionI = '".$cCodificacionI."' WHERE iCodTramite = ".$RsCodTramite['iCodTramite'];
            //$rslUpdate = sqlsrv_query($cnx,$sqlUpdate);
            ///////////////////

            //Ultimo registro de tramite
            $rsUltTra = sqlsrv_query($cnx,"SELECT TOP 1 iCodProyecto FROM Tra_M_Proyecto WHERE iCodTrabajadorRegistro ='".$_SESSION['CODIGO_TRABAJADOR']."' ORDER BY iCodProyecto DESC");
            $RsUltTra = sqlsrv_fetch_array($rsUltTra);
            ...
            if ($_POST['ListaDeCopias'] !== ''){
                $Copia[] = array();
                $Copia   = explode(',',$_POST['ListaDeCopias'][0]);
                //$Copia   = $_POST['ListaDeCopias'];
                $n= count($Copia);
                $h= 0;
            }
            ...
            while ($RsMv = sqlsrv_fetch_array($rsMv)){
                ...
                $sqlPerf = "SELECT iCodPerfil FROM Tra_M_Perfil_Ususario WHERE iCodTrabajador = '".$RsMv['iCodTrabajador']."' AND iCodOficina = '".$_SESSION['iCodOficinaLogin']."'";
                ...
                if($RsPerf['iCodPerfil'] !== 4){
                    ...
                    $sqlAdMv.="(iCodProyecto,iCodTrabajadorRegistro,nFlgTipoDoc,iCodOficinaOrigen,iCodOficinaDerivar,iCodTrabajadorDerivar,iCodIndicacionDerivar,cPrioridadDerivar,cAsuntoDerivar,cObservacionesDerivar,fFecDerivar,fFecMovimiento, nEstadoMovimiento,cFlgTipoMovimiento,cFlgOficina,cCodTipoDocDerivar,iCodTramiteRespuesta,paraAprobar)";
                    $sqlAdMv.=" VALUES ";
                    $sqlAdMv.="('".$RsUltTra['iCodProyecto']."','".$_SESSION['CODIGO_TRABAJADOR']."',2,'".$_SESSION['iCodOficinaLogin']."','".$RsMv['iCodOficina']."', '".$RsMv['iCodTrabajador']."','".$RsMv['iCodIndicacion']."','".$RsMv['cPrioridad']."','".$_POST['cAsunto']."','".$_POST['cObservaciones']."','".$fFecActual."','".$fFecActual."',1,'".$cFlgTipoMovimiento."',1,".$_POST['cCodTipoDoc'].",".$iCodTramiteRespuesta.",1)";
                    $rsAdMv = sqlsrv_query($cnx,$sqlAdMv);

                }else{
                    // $sqlTJefe = " SELECT iCodTrabajador FROM Tra_M_Trabajadores
                    // 							WHERE iCodOficina = '$RsMv['iCodOficina']' AND nFlgEstado = 1 AND iCodCategoria =5 ";
                    $sqlTJefe = "SELECT iCodTrabajador FROM Tra_M_Perfil_Ususario WHERE iCodOficina = '".$RsMv['iCodOficina']."' AND iCodPerfil = 3";
                    ...
                    $sqlAdMv.=" VALUES ";
                    $sqlAdMv.="('".$RsUltTra['iCodProyecto']."','".$_SESSION['CODIGO_TRABAJADOR']."',2,'".$_SESSION['iCodOficinaLogin']."','".$RsMv['iCodOficina']."','".$RsTJefe['iCodTrabajador']."','".$RsMv['iCodTrabajador']."','".$fFecActual."','".$RsMv['iCodIndicacion']."','".$RsMv['iCodIndicacion']."','".$_POST['cObservaciones']."','".$RsMv['cPrioridad']."','".$_POST['cAsunto']."','".$_POST['cObservaciones']."','".$fFecActual."','".$fFecActual."',3,'".$cFlgTipoMovimiento."',1,".$_POST['cCodTipoDoc'].",".$iCodTramiteRespuesta.",1)";
                    $rsAdMv = sqlsrv_query($cnx,$sqlAdMv);
                ...
                while ($RsRefs=sqlsrv_fetch_array($rsRefs)){
                    ...
                    if(sqlsrv_has_rows($rsBusRef)>0){
                        ...
                        if($RsBusRef['nFlgTipoDoc']===1){
                            ...
                            $sqlAdRf.="(iCodTramite,iCodTrabajadorRegistro,nFlgTipoDoc,iCodOficinaOrigen,cCodTipoDocDerivar,iCodOficinaDerivar,iCodTrabajadorDerivar,iCodIndicacionDerivar,cPrioridadDerivar,cAsuntoDerivar,cObservacionesDerivar,fFecDerivar,cReferenciaDerivar,fFecMovimiento,nEstadoMovimiento,cFlgTipoMovimiento,iCodTramiteDerivar,iCodTramiteRespuesta)";
                            $sqlAdRf.=" VALUES ";
                            $sqlAdRf.="('".$RsBusRef['iCodTramite']."','".$_SESSION['CODIGO_TRABAJADOR']."',2,'".$_SESSION['iCodOficinaLogin']."','".$_POST['cCodTipoDoc']."','".$RsMv2['iCodOficina']."','".$RsMv2['iCodTrabajador']."','".$RsMv2['iCodIndicacion']."','".$RsMv2['cPrioridad']."','".$_POST['cAsunto']."','".$_POST['cObservaciones']."','".$fFecActual."','".$cCodificacion."','".$fFecActual."',1,5,'".$RsUltTra['iCodTramite'].",".$iCodTramiteRespuesta."')";
            ...
            // Fin de envío de correo al feje de la oficina
            if ($_POST['esJefe'] === 1) {
            ...
            // SI ES PROFESIONAL Y EL DOCUMENTO ES ESPECIAL
            if ($esProfesional === 1 && $RsEspecial['ES_DOC_ESP'] === 1) {
                $iCodTramite = $nuevoCodigoTramite;
                $fechaDeAprobacion = date('Ymd').' '.date('G:i:s');
                $sqlUpdate = "UPDATE Tra_M_Tramite SET nFlgEnvio = 1,FECHA_DOCUMENTO = '".$fechaDeAprobacion."' WHERE iCodTramite = ".$iCodTramite;
                $rsUpdate = sqlsrv_query($cnx,$sqlUpdate);
            }

            // VIENE DE UN PROFESIONAL QUE QUIERE RESPONDER
            if ($_POST['responder'] === 1) {
                $iCodTramite    = $nuevoCodigoTramite;
                $iCodTramiteSel = $_POST['iCodTramiteSel'];

                $sqlCodMov = "SELECT TOP 1 iCodMovimiento FROM Tra_M_Tramite_Movimientos WHERE iCodtramite = '".$iCodTramiteSel."' ORDER BY iCodMovimiento DESC";
                $rsCodMov  = sqlsrv_query($cnx,$sqlCodMov);
                $RsCodMov  = sqlsrv_fetch_array($rsCodMov);
                $iCodMovimiento = $RsCodMov['iCodMovimiento'];

                $sqpResponder   = "UPDATE Tra_M_Tramite_Movimientos  SET nEstadoMovimiento = 4,  iCodTramiteRespuesta = ".$iCodTramite." WHERE iCodMovimiento = ".$iCodMovimiento;
                $RsResponder = sqlsrv_query($cnx,$sqpResponder);
            }

            // SI ES UN DOCUMENTO DE RESPUESTA LLEGA LA VARIABLE LLENA
            if($iCodTramiteRespuesta !== 0){
                $iCodTramiteAResp    = $nuevoCodigoTramite;


                $sqlCodMov = "SELECT TOP 1 iCodMovimiento FROM Tra_M_Tramite_Movimientos WHERE iCodtramite = '".$iCodTramiteRespuesta."' ORDER BY iCodMovimiento DESC";
                $rsCodMov  = sqlsrv_query($cnx,$sqlCodMov);
                $RsCodMov  = sqlsrv_fetch_array($rsCodMov);
                $iCodMovimiento = $RsCodMov['iCodMovimiento'];

                $sqpResponder   = "UPDATE Tra_M_Tramite_Movimientos  SET nEstadoMovimiento = 4,  iCodTramiteRespuesta = ".$iCodTramiteAResp." WHERE iCodMovimiento = ".$iCodMovimiento;
                $RsResponder = sqlsrv_query($cnx,$sqpResponder);
                //echo $sqpResponder;
            }
            ...
            // Derivar inmediatamente: correo instantaneo
            // -------------------------------------------------------
            //if(rtrim(ltrim($RsUltTra['iCodTramite']))!==''){
            //    $idtramitenew=$RsUltTra['iCodTramite'];
            //    $opc=2; // pro aprobar
            //    include 'email.php';
           // }
            // -------------------------------------------------------
            ...
            header("Location: ../views/registroOficina.php?msg=1");
            ...
            if ($_POST['cCodTipoDoc'] !== '') {
                ...
                $rsEspecial  = sqlsrv_query($cnx,$sqlEspecial);
                $RsEspecial  = sqlsrv_fetch_array($rsEspecial);
                // if ($RsEspecial['ES_DOC_ESP'] == 1) {
                // 	echo "SI ES DOCUMENTO ESPECIAL";
                // }else{
                // 	echo "NO ES DOCUMENTO ESPECIAL";
                // }
            }
            ...
            $RsCorrI = sqlsrv_fetch_array($rsCorrI);
            $CorrelativoAsignarI = $RsCorrI['nCorrelativo'] + 1;
            $rsUpdCorrI = sqlsrv_query($cnx,"UPDATE Tra_M_Correlativo SET nCorrelativo=nCorrelativo+1  WHERE nFlgTipoDoc='9' AND nNumAno='".$nNumAno."'");
            ...
            $rsCodTramite  = sqlsrv_query($cnx,$sqlCodTramite);
            $RsCodTramite  = sqlsrv_fetch_array($rsCodTramite);
            $nuevoCodigoTramite = $RsCodTramite['iCodTramite'];
            ...
            while ($RsMv = sqlsrv_fetch_array($rsMv)){
                ...
                if($RsPerf['iCodPerfil'] !== 4){
                    ...
                    $sqlAdMv.=" VALUES ";
                    $sqlAdMv.="('".$RsUltTra['iCodTramite']."','".$_SESSION['CODIGO_TRABAJADOR']."',2,'".$_SESSION['iCodOficinaLogin']."','".$RsMv['iCodOficina']."', '".$RsMv['iCodTrabajador']."','".$RsMv['iCodIndicacion']."','".$RsMv['cPrioridad']."','".$_POST['cAsunto']."','".$_POST['cObservaciones']."','".$fFecActual."','".$fFecActual."',1,'".$cFlgTipoMovimiento."',1,".$_POST['cCodTipoDoc'].",".$iCodTramiteRespuesta.")";
                    $rsAdMv = sqlsrv_query($cnx,$sqlAdMv);
            ...
            $fFecActual=date('d-m-Y G:i');
             ...
             header("Location: ../views/bandeja-de-pendientes.php?msg=1");
            /*echo "<html>";
            echo "<head>";
            echo "</head>";
            echo "<body OnLoad=\"document.form_envio.submit();\">";
            echo "<form method=\"POST\" name=\"form_envio\" action=\"registroDocumentoOficinaInterno.php\">";
    ...
    case 4:
        ...
        echo "<input type='hidden' name='nFlgTipoDoc' value=2>";
        echo "<input type='hidden' name='fFecActual' value='".$fFecActual."' >";
		if($nFlgRestricUp??''===1){
			echo "<input type='hidden' name='nFlgRestricUp' value='1'>";
		}		
	...
	case 5:
	  ...
	  //Ultimo registro de tramite
        ...
        $sqlAdMv.=" VALUES ";
        $sqlAdMv.="('".$RsUltTra['iCodTramite']."','".$_SESSION['CODIGO_TRABAJADOR']."',3,'".$_SESSION['iCodOficinaLogin']."',0,3,'".$_POST['cAsunto']."','".$_POST['cObservaciones']."','".$fFecActual."','".$fFecActual."',1,1,1)";
        $rsAdMv = sqlsrv_query($cnx,$sqlAdMv);
		...
		echo "<input type='hidden' name='iCodTramite' value='".$nuevoCodigoTramite."'>";
		echo "</form>";
		echo "</body>";
		echo "</html>";
	break;
	...
	case 7: // registro anexo
		...
		//  Sql es ejecutado en SP
		$sqlAdd.="SP_DOC_ANEXO_INSERT '".$cCodificacion."',	'".$_SESSION['CODIGO_TRABAJADOR']."', '".$_SESSION['iCodOficinaLogin']."', '".$_POST['cCodTipoDoc']."', '".$_POST['cNroDocumento']."', '".$_POST['cAsunto']."', '".$_POST['iCodRemitente']."', '".$_POST['cNomRemite']."', '".$_POST['cObservaciones']."', '".$_POST['nNumFolio']."', '".$fFecActual."', '".$fFecActual."', '".$_POST['iCodTramite']."', '".$nCodBarra."','".$_POST['fechaDocumento']."' ";
		...
		if ($_FILES['fileUpLoadDigital']['name'] !== ''){
			...
			$rsUltDoc = sqlsrv_query($cnx,"SELECT TOP 1 iCodDigital FROM Tra_M_Tramite_Digitales WHERE iCodTramite = '".$RsUltTra['iCodTramite']."' ORDER BY iCodTramite DESC");
...
//    $sqlUpd.="WHERE iCodTramite='$_POST[iCodTramite]'";
		...
		$countCodTupaRequisito = count ($_POST['iCodTupaRequisito']);
		For ($h=0;$h<$countCodTupaRequisito;$h++){
      	    $iCodTupaRequisito= $_POST['iCodTupaRequisito'];
			$sqlIns="INSERT INTO Tra_M_Tramite_Requisitos (iCodTupaRequisito, iCodTramite) VALUES ('".$iCodTupaRequisito[$h]."', '".$_POST['iCodTramite']."') ";
   			$rsIns=sqlsrv_query($cnx,$sqlIns);
		}
		
		if($_POST['iCodOficinaResponsable']!=='' && $_POST['numMov']===0){
            $sqlMov="INSERT INTO Tra_M_Tramite_Movimientos ";
	...
	//--- ACTUALIZAR TRAMITE SIN TUPA ---//////////////////////////////////////////////////////////
	case 9:
        //  Sql es ejecutado en SP
        $fFecActual = date('Ymd', strtotime($_POST['fFecRegistro'])).' '.date('G:i',strtotime($_POST['fFecRegistro']));
	    //$fFecActual
	    $fFecActual2 = date('Ymd').' '.date('G:i:s');
	    ...
	    $sqlUpd="SP_DOC_ENTRADA_SIN_TUPA_UPDATE '".$_POST['cCodTipoDoc']."', '".$cNroDocumento."', '".$_POST['iCodRemitente']."', '".$cAsunto."', '".$cObservaciones."', '".$cReferencia."',  '".$_POST['iCodIndicacion']."','".$nNumFolio."', '".$_POST['nTiempoRespuesta']."', '".$cNomRemite."', '".$fFecActual."','".$fFecActual2."','".$_POST['iCodTramite']."','".$mantenerPendiente."'";
		...
		if($_POST['iCodOficinaResponsable']!=='' && $_POST['numMov']===0){
			...
			$sqlMov.="(iCodTramite,iCodTrabajadorRegistro,nFlgTipoDoc,iCodOficinaOrigen,iCodOficinaDerivar,iCodTrabajadorDerivar,cCodTipoDocDerivar,iCodIndicacionDerivar,fFecDerivar,nEstadoMovimiento, fFecMovimiento, cFlgTipoMovimiento)";
			$sqlMov.=" VALUES ";
			$sqlMov.="('".$_POST['iCodTramite']."','".$_SESSION['CODIGO_TRABAJADOR']."',1,'".$_SESSION['iCodOficinaLogin']."','".$_POST['iCodOficinaResponsable']."','".$_POST['iCodTrabajadorResponsable']."','".$_POST['cCodTipoDoc']."','".$_POST['iCodIndicacion']."','".$fFecActual2."', 1,'".$fFecActual."',1)";
			$rsMov=sqlsrv_query($cnx,$sqlMov);
   	    }
        ...
        $sqlUpdMovE=" SP_DOC_ENTRADA_MOV_UPDATE '".$_POST['cCodTipoDoc']."', '".$cAsunto."', '".$cObservaciones."','".$fFecActual2."', '".$RsUpdMov['iCodMovimiento']."' ";
        ...
        // -------------------------------------------------------
        if ($_POST['ActivarDestino'] === 1) {
            $idtramitenew   =   $_POST['iCodTramite'];
            $responsable    =   $_POST['iCodTrabajadorResponsable'];
            $tipodocumento  =   $_POST['cCodTipoDoc'];
	...
	case 11:
        ...
        if(($_POST['nFlgEnvio']??'')==1){
            ...
            $rsUpdEnvio=sqlsrv_query($cnx,$sqlUpdEnvio);
            $sqlMov="INSERT INTO Tra_M_Tramite_Movimientos ";
                $sqlMov.="(iCodTramite, iCodTrabajadorRegistro,nFlgTipoDoc,  iCodOficinaOrigen,iCodOficinaDerivar,iCodTrabajadorDerivar,  
                cCodTipoDocDerivar,  cAsuntoDerivar, cObservacionesDerivar,   iCodDigital,iCodTramiteRel,fFecDerivar,  nEstadoMovimiento, 
                fFecMovimiento, nFlgEnvio,   cFlgTipoMovimiento)";
             $sqlMov.=" VALUES ";
             $sqlMov.="('".($_POST['iCodTramite'])."', '".$_SESSION['CODIGO_TRABAJADOR']."', 4,
             '".$_SESSION['iCodOficinaLogin']."', '".($_POST['iCodOficinaResponsable']??'')."', '".($_POST['iCodTrabajadorResponsable']??'')."', '".($_POST['cCodTipoDoc']??'')."', 
             '".($_POST['cAsunto']??'')."', '".($_POST['cObservaciones']??'')."', '".($RsUltDig['iCodDigital']??'')."', '".($_POST['iCodTramite']??'')."', '$fFecActual', 1,                
             '$fFecActual',1,3)";
             $rsMov=sqlsrv_query($cnx,$sqlMov);
        }
	
        //Actualizar Flujo
        $sqlDetMovF="UPDATE Tra_M_Tramite_Movimientos SET cAsuntoDerivar= '".($_POST['cAsunto']??'')."', cObservacionesDerivar= '".($_POST['cObservaciones']??'')."' 
        WHERE iCodTramite = '".($_POST['iCodTramite']??'')."' 
        AND cFlgTipoMovimiento= 3 AND  nFlgTipoDoc = 4  ";
        $rsDetMovF=sqlsrv_query($cnx,$sqlDetMovF);
   	
        if(($_POST['iCodOficinaResponsable']??'')!=($_POST['iCodOfi']??0)){
           $rsMovA=sqlsrv_query($cnx,"UPDATE Tra_M_Tramite_Movimientos SET iCodOficinaDerivar='".($_POST['iCodOficinaResponsable']??'')."' 
            WHERE iCodMovimiento='".($_POST['iCodMovimiento']??'')."'");
        }
        if(($_POST['iCodTrabajadorResponsable']??'')!=($_POST['iCodTra']??'')){
          $rsMovB=sqlsrv_query($cnx,"UPDATE Tra_M_Tramite_Movimientos SET iCodTrabajadorDerivar='".($_POST['iCodTrabajadorResponsable']??'')."' 
          WHERE iCodMovimiento='".($_POST['iCodMovimiento']??'')."'");
        }
		echo "<html>";
		echo "<head>";
	...
	case 12:
		...
		for($i=1; $i<=$_POST['CantCopias']; $i++){
			...
			$cObservacionesSelect=$cObservaciones[$recolector];
  		    ...
  		    $sqlMov="INSERT INTO Tra_M_Tramite_Movimientos ";
			$sqlMov.="(iCodTramite,iCodTrabajadorRegistro, nFlgTipoDoc,  iCodOficinaOrigen,  iCodOficinaDerivar, iCodTrabajadorDerivar, 
			cAsuntoDerivar,  cObservacionesDerivar, cCodTipoDocDerivar,  iCodIndicacionDerivar,   fFecDerivar,   nEstadoMovimiento, 
			fFecMovimiento, nFlgEnvio, cFlgTipoMovimiento)";
			$sqlMov.=" VALUES ";
	...
	case 13: // actualizar interno oficina
		...
		if(empty($_FILES['fileUpLoadDigital']['name'])){
  		...
  		$num = count($extension)-1;
			...
			if($extension[$num] == "exe" OR $extension[$num] == "dll" OR $extension[$num] == "EXE" OR $extension[$num] == "DLL"){
                ...
                $nuevo_nombre = $cDescTipoDoc."-".str_replace("/","-",$_POST['cCodificacion']).".".trim($extension[$num]);
                ...
                $sqlBusRef = "SELECT * FROM Tra_M_Tramite WHERE cCodificacion='".$RsRefs['cReferencia']."'";
                    ...
                    if(sqlsrv_has_rows($rsBusRef)>0){
                        ...
                        if($RsBusRef[nFlgTipoDoc] == 1){
                            ...
                            $sqlAdRf = "INSERT INTO Tra_M_Tramite_Movimientos ";
                            $sqlAdRf.="(iCodTramite,iCodTrabajadorRegistro,nFlgTipoDoc, iCodOficinaOrigen,cCodTipoDocDerivar,iCodOficinaDerivar,
                                                iCodTrabajadorDerivar,iCodIndicacionDerivar,cPrioridadDerivar,cAsuntoDerivar,cObservacionesDerivar,fFecDerivar,nFlgEnvio,cReferenciaDerivar,fFecMovimiento,nEstadoMovimiento,cFlgTipoMovimiento,iCodTramiteDerivar)";
                            $sqlAdRf.=" VALUES ";
        ...
        $sql = "SELECT nFlgEnvio FROM Tra_M_Tramite WHERE iCodTramite = ".$_POST['iCodTramite'];
        $rs  = sqlsrv_query($cnx,$sql);
        $Rs  = sqlsrv_fetch_object($rs);
        if ($Rs->nFlgEnvio == 2) {
        $updateEstado = "UPDATE Tra_M_Tramite
                                         SET nFlgEnvio = 0
                                         WHERE iCodTramite = ".$_POST['iCodTramite'];
        $rsUpdateEstado = sqlsrv_query($cnx,$updateEstado);
      }
    ...
    case 21: //añadir referencia temporal
	    ...
	    $sqlAdd.="('$_POST[iCodTramiteRef]','$_POST[cReferencia]', '$_POST[iCodTramite]','$_SESSION[cCodRef]', 'PENDIENTE', 1)";
          ...
          if (($_POST['sal']??'') == 3){
              echo "<form method=POST name=form_envio action=registroSalida.php>";
          }else  if (($_POST['sal']??'') == 4){
              echo "<form method=POST name=form_envio action=registroEspecial.php>";
              ...
              echo "<input type=hidden name=iCodTupa value=\"".($_POST['iCodTupa']??'')."\">";
              echo "<input type=hidden name=cCodTipoDoc value=\"".($_POST['cCodTipoDoc']??'')."\">";
              echo "<input type=hidden name=iCodTramiteRef value=\"".($_POST['iCodTramiteRef']??'')."\">";
              echo "<input type=hidden name=cReferencia value=\"".($_POST['cReferencia']??'')."\">";
      ...
      case 25: //añadir movimiento temporal
          ...
          if(trim($_POST['iCodOficina'])!=""){
              echo "<form method=POST name=form_envio action=pendientesControlDerivarAdm.php?iCodMovimientoAccion=".isset($_POST['iCodMovimiento'])."&iCodOficina=".isset($_POST['iCodOficina'])."&clear=1#area>";
          }else{
      ...
      case 30: // Subir Digital - interno oficina
          ...
          break;
	/*******************************************************************************************************/
      case 31: /* REGISTRO SIN TUPA EXTERNO */
          if ($_SESSION['cCodRef'] == ""){
              $Fecha = date("Ymd-Gis");
              $_SESSION['cCodRef'] = $_SESSION['CODIGO_TRABAJADOR']."-".$_SESSION['iCodOficinaLogin']."-".$Fecha;
          }
          ...
          $rsCorr = sqlsrv_query($cnx,"SELECT TOP 1 nCorrelativo FROM Tra_M_Correlativo 
          ...
          if ($_POST['nFlgClaseDoc'] == 2){ //sql sin tupa
              ...
              $sqlAdd.="SP_DOC_ENTRADA_SIN_TUPA_INSERT '$cCodificacion', '".$_SESSION['CODIGO_TRABAJADOR']."', '".$_SESSION['iCodOficinaLogin']."', '".$_POST['cCodTipoDoc']."', '$fFecActual', '$cNroDocumento', '".$_POST['iCodRemitente']."', '$cNomRemite', '$cAsunto', '$cObservaciones', '$cReferencia', NULL, '$nNumFolio', '".$_POST['nTiempoRespuesta']."', '$mantenerPendiente','$fFecActual',  '$nCodBarra', '$cPassword','$fechamax','$archivoFisico','$clave'";
          ...
          if ($conteo2 >= 1){
              $sqlTraF = "SELECT TOP 1 iCodTramite FROM Tra_M_Tramite 
									  WHERE iCodTrabajadorRegistro='".$_SESSION['CODIGO_TRABAJADOR']."' order by fFecRegistro desc";
              $rsTraf1 = sqlsrv_query($cnx,$sqlTraF);
              $RsTraf2 = sqlsrv_fetch_array($rsTraf1);
