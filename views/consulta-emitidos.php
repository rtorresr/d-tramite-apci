...
if($_SESSION['CODIGO_TRABAJADOR']!=""){
    ...
    <main>
        ...
        <<div id="modalPrimer" class="modal modal-fixed-header modal-fixed-footer">
            <div class="modal-header">
                <h4>Tipo documento</h4>
            </div>
            <div class="modal-content">
                <div class="col s6 input-field">
                    <select id="cCodTipoDocPrimera" name="cCodTipoDoc">
                    </select>
                    <label for="cCodTipoDocPrimera" class="active">Tipo de documento</label>
                </div>
            </div>
            <div class="modal-footer">
                <a id="btnDocPrimerModal" class="btn btn-link"><i class="fas fa-search left"></i><span>Buscar</span></a>
                <a class="modal-close waves-effect waves-green btn-flat">Cerrar</a>
            </div>
        </div>
    ...
    <script>
        ...
        $(document).ready(function (){
            ...
            let elem = document.querySelector('#modalPrimer');
            ...
            var tblConsultaEmit = $('#tblConsultaEmit').DataTable({
                ...
                ajax: {
                    url: 'ajaxtablas/ajaxConsultaEmitidos.php',
                    type: 'POST',
                    datatype: 'json',
                    data: function ( d ) {
                        return $.extend( {}, d, {
                            "VI_NCUD": $('#txtCUD').val()==null?"":$('#txtCUD').val(),
                            "VI_ICODOFICINA": $('#cCodOfi').val()==null?0:$('#cCodOfi').val(),
            ...
            $('#txtCUD').keydown(function (e) {
                if(e.which === 13){
                    e.preventDefault();
                    DrawTable(tblConsultaEmit, 'modalFiltros');
                }
            });
            ...
            $('.btnClear').click(function(e){
                ...
                ClearForm(frmConsultaEmit, tblConsultaEmit, 'modalFiltros');
            });

            $('#btnDocPrimerModal').click(function (e) {
                e.preventDefault();
                $('#cCodTipoDoc').val($('#cCodTipoDocPrimera').val());
                DrawTable(tblConsultaEmit, 'modalFiltros');
            ...
            btnDoc.on('click', function(e) {
                ...
                let fila = tblConsultaEmit.rows( { selected: true } ).data().toArray()[0];
