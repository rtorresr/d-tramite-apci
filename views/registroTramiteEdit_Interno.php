...
document.frmEdicion.action="<?=$_SERVER['PHP_SELF']?>?iCodTramite=<?=$_GET[iCodTramite]?>&URI=<?=$_GET[URI]?>#area";
document.frmEdicion.submit();
return false;
}

function releer(){
  document.frmEdicion.action="<?=$_SERVER['PHP_SELF']?>?iCodTramite=<?=$_GET[iCodTramite]?>&URI=<?=$_GET[URI]?>&clear=1#area";
  document.frmEdicion.submit();
}
...
function AddReferencia(){
	if (document.frmEdicion.cReferencia.value.length == "")
  {
    alert("Ingrese Referencia");
    document.frmEdicion.cReferencia.focus();
    return (false);
  }
  document.frmEdicion.opcion.value=21;
  document.frmEdicion.action="registroDataEdicion.php";
  document.frmEdicion.submit();
}
	  ...
	  <td>
        ...
        <tr>
		  ...
		  <tr>
			...
			<tr>
			<td valign="top"  width="135">Tipo de Documento:</td>
          ...
          sqlsrv_free_stmt($rsTipo);
			...
			<td  width="135">N&ordm; del Tramite:</td>
			...
			<tr>
			<td valign="top"  width="135">Asunto, Asunto:</td>
			<td colspan="2"  align="left">
						<textarea name="cAsunto" style="width:340px;height:55px" class="FormPropertReg form-control"><?php if($_GET[clear]==""){ echo trim($RsCod['cAsunto']); }Else{ echo $_POST['cAsunto'];}?></textarea>&nbsp;<span class="FormCellRequisito"></span>
			</td>
			<td valign="top"  width="135">Observaciones:</td>
			<td colspan="2"  align="left" valign="top">
			...
			<tr>
			<td valign="top"  width="135">Folios:</td>
			<td colspan="2"  align="left" valign="top"><input type="text" name="nNumFolio" value="<?php if($_GET[clear]==""){ echo trim($Rs[nNumFolio]); }Else{ echo $_POST[nNumFolio];}?>" class="FormPropertReg form-control" style="width:40px;text-align:right"  maxlength="3" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;"/>&nbsp;<span class="FormCellRequisito"></span></td>
			<td valign="top" >Referencia(s):</td>
			<td colspan="2"  align="left">
					<table><tr>
					<td align="center"><input type="text" readonly="readonly" name="cReferencia" value="<?php if($_GET[clear]==""){ echo trim($RsCod[cReferencia]); }Else{ echo trim($_POST[cReferencia]);}?>" class="FormPropertReg form-control" style="width:140px;text-transform:uppercase" />
			...
			<tr>
				<td valign="top"  width="135">Adjuntar Archivo:</td>
          	...
          	if(sqlsrv_has_rows($rsDig)>0){
				...
				<td colspan="2"  align="left"><input type="text" style="width:60px;text-transform:uppercase" name="cSiglaAutor" value="<?php if($_GET[clear]==""){ echo $Rs[cSiglaAutor]; }Else{ echo $_POST[cSiglaAutor];}?>" class="FormPropertReg form-control"  /></td>
			</tr>
          ...
          <td valign="top"><input type="radio" name="radioMultiple" onclick="activaMultiple();" <?php if($_POST[radioSeleccion]==2) echo "checked"?> onChange="releer();">M�ltiple</td>
              ...
              <option <?php if($_POST[cPrioridad]=="Baja") echo "selected"?> value="Baja">Baja</option>
					...
					<div style="display:none" id="areaMultiple">
							<table><tr>
							<td align="center"><div class="btn btn-primary" style="width:130px;height:17px;padding-top:4px;"><a style=" text-decoration:none" href="registroOficinaAdLs.php?iCodTramite=<?=$_POST[iCodTramite]?>&nFlgTipoDoc=<?=$_POST[nFlgTipoDoc]?>&iCodTrabajadorRegistro=<?=$_POST[iCodTrabajadorRegistro]?>&iCodOficinaRegistro=<?=$_POST[iCodOficinaRegistro]?>&iCodTrabajadorSolicitado=<?=$_POST[iCodTrabajadorSolicitado]?>&fFecDocumento=<?=$_POST['fFecDocumento']?>&fFecRegistro=<?=$_POST['fFecRegistro']?>&cCodTipoDoc=<?=$_POST[cCodTipoDoc]?>&cCodificacion=<?=$_POST[cCodificacion]?>&cAsunto=<?=$_POST['cAsunto']?>&cObservaciones=<?=$_POST[cObservaciones]?>&nNumFolio=<?=$_POST[nNumFolio]?>&cReferencia=<?=$_POST[cReferencia]?>&iCodTramiteRef=<?=$_POST[iCodTramiteRef]?>&cSiglaAutor=<?=$_POST[cSiglaAutor]?>&nFlgEnvio=<?=$_POST[nFlgEnvio]?>&URI=<?=$_GET[URI]?>" rel="lyteframe" title="Lista de Oficinas" rev="width: 500px; height: 550px; scrolling: auto; border:no">Seleccionar Oficinas</a> </div></td>
							</tr></table>
					</div>					
			...
			<td colspan="6" align="left">
					...
					<?
					$sqlMovs="SELECT * FROM Tra_M_Tramite_Movimientos WHERE iCodTramite='$RsCod[iCodTramite]' AND cFlgOficina=1 ORDER BY iCodMovimiento ASC";
          ...
          echo $RsOfc["cNomOficina"];
            ...
            echo $RsInd["cIndicacion"];
					...
					<td align="center">
						<a href="registroDataEdicion.php?iCodMovimiento=<?=$RsMovs[iCodMovimiento]?>&iCodTramite=<?=$RsCod[iCodTramite]?>&opcion=7&cCodTipoDoc=<?php if($_GET[clear]==""){ echo $RsCod[cCodTipoDoc]; }Else{ echo $_POST[cCodTipoDoc];}?>&iCodTrabajadorSolicitado=<?php if($_GET[clear]==""){ echo $RsCod[iCodTrabajadorSolicitado]; }Else{ echo $_POST[iCodTrabajadorSolicitado]; }?>&cReferencia=<?php if($_GET[clear]==""){ echo $RsCod[cReferencia]; }Else{ echo $_POST[cReferencia];}?>&cAsunto=<?php if($_GET[clear]==""){ echo $RsCod['cAsunto']; }Else{ echo $_POST['cAsunto']; }?>&cObservaciones=<?php if($_GET[clear]==""){ echo $RsCod[cObservaciones]; }Else{ echo $_POST[cObservaciones]; }?>&iCodIndicacion=<?php if($_GET[clear]==""){ echo $RsCod[iCodIndicacion]; }Else{ echo $_POST[iCodIndicacion];}?>&nFlgRpta=<?php if($_GET[clear]==""){ echo $RsCod[nFlgRpta]; }Else{ echo $_POST[nFlgRpta];}?>&nNumFolio=<?php if($_GET[clear]==""){ echo $RsCod[nNumFolio]; }Else{ echo $_POST[nNumFolio];}?>&fFecPlazo=<?php if($_GET[clear]==""){ echo $RsCod[fFecPlazo]; }Else{ echo $_POST[fFecPlazo]; }?>&nFlgEnvio=<?php if($_GET[clear]==""){ echo $RsCod[nFlgEnvio]; }Else{ echo $_POST[nFlgEnvio]; }?>&nFlgTipoDoc=<?php if($_GET[clear]==""){ echo $RsCod[nFlgTipoDoc]; }Else{ echo $_POST[nFlgTipoDoc]; }?>&iCodTrabajadorRegistro=<?php if($_GET[clear]==""){ echo $RsCod[iCodTrabajadorRegistro]; }Else{ echo $_POST[iCodTrabajadorRegistro]; }?>&iCodOficinaRegistro=<?php if($_GET[clear]==""){ echo $RsCod[iCodOficinaRegistro]; }Else{ echo $_POST[iCodOficinaRegistro]; }?>&fFecDocumento=<?php if($_GET[clear]==""){ echo $RsCod['fFecDocumento']; }Else{ echo $_POST['fFecDocumento']; }?>&fFecRegistro=<?php if($_GET[clear]==""){ echo $RsCod['fFecRegistro']; }Else{ echo $_POST['fFecRegistro']; }?>& cCodificacion=<?php if($_GET[clear]==""){ echo $RsCod[cCodificacion]; }Else{ echo $_POST[cCodificacion]; }?>&iCodTramiteRef=<?php if($_GET[clear]==""){ echo $RsCod[iCodTramiteRef]; }Else{ echo $_POST[iCodTramiteRef]; }?>&cSiglaAutor=<?php if($_GET[clear]==""){ echo $RsCod[cSiglaAutor]; }Else{ echo $_POST[cSiglaAutor]; }?>&
URI=<?=$_GET[URI]?>"><img src="images/icon_del.png" border="0" width="16" height="16"></a>
