...
<div class="AreaTitulo">Consulta >> Control Prestamo </div>
						...
						<tr>
							...
							<td align="left"><input type="txt" name="cCodificacion" value="<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>" size="28" class="FormPropertReg form-control" onKeypress="if (event.keyCode < 45 || event.keyCode > 57 || event.keyCode == 47 || event.keyCode == 46  ) event.returnValue = false;" ></td>
							<td >
							    Estado:
							</td>
							<td>
							    <select name="estado">
							        ...
							        ?>
							        <option value="1" <?php echo $a1;?>>Pendiente</option>
							        <option value="2" <?php echo $a2;?>>Entregado</option>
							    </select>
							</td>
...
if($orden=="DESC") $cambio="ASC";
  ...
  if($_GET[cReferenciaPCM]=="" or $fecini!="" or $fecfin!="" or $_GET['cCodificacion']!="" or $_GET['iCodOficina']!=""  or $_GET['cAsunto']!="" or $_GET[cReferencia]!="" or $_GET['iCodTupa']!="" or $_GET['cNombre']!="" or $_GET[iCodTrabajadoResponsable]!="" or $_GET['cNroDocumento']!="" or $_GET[cNomRemite]!=""){
    ...
    if($_GET['cCodificacion']!=''){
        ...
        $sql.=" where Cod_estado_prestamo='".$_GET['estado']."'";
    }else{
   ...
   $total = sqlsrv_has_rows($rs);
    ...
    <tr>
        <td width="10%" class="headCellColum"> ID  </td>
        <td width="20%" class="headCellColum">Oficina / Solicitado Por.</td> 
        <td width="30%" class="headCellColum">Documento Solicitado<br>Tipo Documento<br>Remitente</td>
        <td width="30%" class="headCellColum">Asunto</td>
        <td width="20%" class="headCellColum">Fecha Solicitud <br> Fecha Entrega</td>
        <td width="10%" class="headCellColum">Estado </td>
        <td width="10%" class="headCellColum">Accion </td>
    </tr>
    <?php
    $coo=0;
    do{
    ...
    <tr bgcolor="<?=$color?>" onMouseOver="this.style.backgroundColor='#BFDEFF'" OnMouseOut="this.style.backgroundColor='<?=$color?>'">
        <td align='center'>
            <b class="a1"><?php echo $rs[codigo];?></b>
        ...
        do{
            echo "<b class='a2'>".$rsx1['cNomOficina']."</b>";
        }while($rsx1=sqlsrv_fetch_array($queryx1));
        ?>
        <br>
        <?php
        $sqlx1a= "select * from Tra_M_Trabajadores where iCodTrabajador='".$rs[solicitadoPor]."'";
        $queryx1a=sqlsrv_query($cnx,$sqlx1a);
        $rsx1a=sqlsrv_fetch_array($queryx1a);
        do{
            echo "<b class='b1'>".rtrim(ltrim($rsx1a['cApellidosTrabajador']))." ".rtrim(ltrim($rsx1a['cNombresTrabajador']))."</b>";
        }while($rsx1a=sqlsrv_fetch_array($queryx1a));
        ?>
        
        </td>
       
        <td valign='top'>
            <b class="c1"><?php echo $rs[cod_documento];?></b>	
            <br>
        
        <?php 
        
        // remitente
        $sqlxa= "select (select cDescTipoDoc from Tra_M_Tipo_Documento where cCodTipoDoc=t.cCodTipoDoc) as cCodTipoDoc from Tra_M_Tramite t
where iCodTramite='".$rs[icodtramite]."'";
        $queryxa=sqlsrv_query($cnx,$sqlxa);
        $rsxa=sqlsrv_fetch_array($queryxa);

        do{
            echo "<b class='c2'>".$rsxa['cCodTipoDoc']."</b>";
        }while($rsxa=sqlsrv_fetch_array($queryxa));
        ?> 
        <br>
        <?php 
        ...
        do{
            echo "<b class='c4'>".$rsx['iCodRemitentex']."</b>";
        ...
        <b class="a1">F. Solicitud<br><?php echo fechas($rs[fecha_solicitud]);?></b>
        
        <?php if(fechas($rs[fecha_entrega])!=''){ ?>
        <br>
        
        <b class="e2">
           F. Entrega<br> <?php echo fechas($rs[fecha_entrega]);?>
        </b>
        <?php } ?> 
        ...
        <td>
           ...
           <?php 
            if($rs[Cod_estado_prestamo]==1){
                echo "<b class='pendiente'>Pendiente</b>";
            }else{
                echo "<b class='entregado'>Entregado</b>";
            }
            ?>	
           ...
           <?php 
            if($rs[Cod_estado_prestamo]==1){
                echo "<a href='pestamoDOCUMENTARIO DIGITALdata.php?id=".$rs[codigo]."&estado=2' class='cambio' onclick='return confirmarx()'><B>Entregar</b></a>";
            }else{
                echo "<a href='pestamoDOCUMENTARIO DIGITALdata.php?id=".$rs[codigo]."&estado=3' class='cambio' onclick='return confirmar()'><b>Devolver</b></a>";
            }
            ?>	
            ...
            <script>
            function confirmarx(){
                if(confirm('Esta seguro de Entregar Prestamo?'))
                    return true;
                else
                    return false;
            }
            function confirmar(){
                if(confirm('Esta seguro de Devolver Prestamo?'))
                    return true;
                else
                    return false;
            }
