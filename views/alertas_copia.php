...
$sqlVCTupa.="WHERE Tra_M_Tramite.nFlgEnvio=1 AND Tra_M_Tramite_Movimientos.fFecRecepcion IS NOT NULL AND Tra_M_Tramite.nFlgTipoDoc=1 AND Tra_M_Tramite_Movimientos.cFlgTipoMovimiento!=5 AND Tra_M_Tramite.nFlgEstado=2";
			      ...
			      $RsTipDoc=sqlsrv_fetch_array($rsTipDoc);
      	 		...
      	 		$mensajeH ='<html>';
						...
						if(($RsVCTupa[nTiempoRespuesta]-$difDiasCnTupa)==0){
							$mensajeH.='Ha recibido una notificaci�n generada automaticamente, indicando que el Documento <b>ha vencido</b></b>:<br><br>';
						}
						$mensajeH.='<table>';
						...
						If($correoEnviar!=""){
      					define('DISPLAY_XPM4_ERRORS', true);
								...
								//$c = $m->Connect('mail.llampanet.net', 25, 'test@llampanet.net', 'test') or die(print_r($m->Result));
								$c = $m->Connect('192.168.145.10', 25, 'tramiteDOCUMENTARIO DIGITAL@pcm.gob.pe', 'tramite23') or die(print_r($m->Result));
								$m->Send($c);
								$m->Disconnect();
