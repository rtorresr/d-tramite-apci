...
<div class="AreaTitulo">Consulta >> Doc. Interno Oficina</div>
						...
						<tr>
                           ...
                           <tr>
                            <td align="left">
	                          ...
	                          | &nbsp; &nbsp;  Editar &nbsp; <i class="fas fa-edit"></i>&nbsp;&nbsp;&nbsp; Hoja de Tr&aacute;mite &nbsp;<img src="images/icon_print.png" width="16" height="16" border="0"></td>
                           </tr>
                          </table>
                          </td>
...
if($orden=="DESC") $cambio="ASC";
   ...
   if( $fecfinx!=''){
	...
	$sql.= " SP_CONSULTA_INTERNO_OFICINA_PROFESIONAL '$fecinix', '$fecfinx',  '$SIx', '$NOx','%$cCodificacionx%', '%$cAsuntox%',  '%$cObservacionesx%', '$cCodTipoDocx' ,'$iCodOficinax','$OfiLog', '$campox', '$ordenx', $_SESSION['CODIGO_TRABAJADOR'] ";
...
if($_GET['cCodificacion']=="" && $fecinix=="" && $fecfinx=="" && $_GET['cCodTipoDoc']=="" && $_GET['cAsunto']=="" &&  $_GET['iCodOficina']=="" && $_GET[SI]=="" && $_GET[NO]=="" ){
   $sqlin= " SP_CONSULTA_INTERNO_OFICINA_LISTA_PROFESIONAL '$OfiLog', '".$_SESSION['CODIGO_TRABAJADOR']."' ";
...
*/?>    
    ...
    //echo "<a href=\"registroOficinaEdit.php?iCodTramite=".$Rs[iCodTramite]."&URI=".$_SERVER['REQUEST_URI']."\"><img src=\"images/icon_edit.png\" width=\"16\" height=\"16\" alt=\"Editar Documento\" border=\"0\"></a>";
    		...
    		$sqlRep="SELECT fFecRecepcion FROM Tra_M_Tramite_Movimientos WHERE iCodTramite=$Rs[iCodTramite]  AND (fFecRecepcion IS NOT NULL or fFecRecepcion!='')";
			$rsRep=sqlsrv_query($cnx,$sqlRep);
			$sqlDev="SELECT iCodTramiteDerivar FROM Tra_M_Tramite_Movimientos WHERE iCodTramiteDerivar = $Rs[iCodTramite] and cFlgTipoMovimiento != 5 ";
			$rsDev=sqlsrv_query($cnx,$sqlDev);
			
			if($_SESSION['CODIGO_TRABAJADOR']==10576){
			echo "<a href=\"registroOficinaDownLoad.php?iCodTramite=".$Rs[iCodTramite]."&URI=".$_SERVER['REQUEST_URI']."\"><img src=\"images/icon_download_1.png\" width=\"16\" height=\"16\" alt=\"Subir Documento\" border=\"0\"></a>";
			
			}
					if(sqlsrv_has_rows($rsRep)==0){
				
				if(sqlsrv_has_rows($rsDev)==0)	{
      		/*if(sqlsrv_has_rows($rsEdit)==sqlsrv_has_rows($rsEdit2)){ */
		 		echo "<a href=\"registroOficinaEdit.php?iCodTramite=".$Rs[iCodTramite]."&URI=".$_SERVER['REQUEST_URI']."\"><img src=\"images/icon_edit.png\" width=\"16\" height=\"16\" alt=\"Editar Documento\" border=\"0\"></a>";
			/*}*/ 
		 		if($RsCorre[nCorrelativo]==$Correlativo){			
						echo "&nbsp;<a href=\"registroData.php?iCodTramite=".$Rs[iCodTramite]."&URI=".$_SERVER['REQUEST_URI']."&iCodCorrelativo=".$RsCorre[iCodCorrelativo]."&nCorrelativo=".($Correlativo-1)."&opcion=23\" onClick=\"return ConfirmarBorrado();\"><img src=\"images/icon_del.png\" width=\"16\" height=\"16\" alt=\"Editar Documento\" border=\"0\"></a>";
						}
					}
				}
	   ?>
       <!--div onClick="window.open('registroHojaHistorica.php?cCodificacion=<?=trim($Rs[cCodificacion])?>&iCodTramite=<?=trim($Rs[iCodTramite])?>','_blank')"  onMouseOver="this.style.cursor='hand'"><img src=images/icon_print.png border=0 width=16 height=16 alt="Hoja de TRÁMITE"></div-->
    </td>
	</tr>
