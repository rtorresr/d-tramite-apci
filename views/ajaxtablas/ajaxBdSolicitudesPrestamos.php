...
$params = [
    ...
    ,$_POST['FlgArchivoCentral']
    ,$_SESSION['IdSesion']
];

$sql = "{call UP_SOLICITUDES_PRESTAMOS_RECIBIDOS (?,?,?)}";
...
while($Rs=sqlsrv_fetch_array($rs, SQLSRV_FETCH_ASSOC)){
    ...
    $subdata['IdArchivoCargoPrestamo'] = $Rs['IdArchivoCargoPrestamo'];
    $subdata['IdArchivoCargoDevolucion'] = $Rs['IdArchivoCargoDevolucion'];
    $data[]=$subdata;
    $contador++;
}
