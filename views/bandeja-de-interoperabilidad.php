...
$pageTitle = "Bandeja de Interoperabilidad";
$activeItem = "bandeja-de-interoperabilidad.php";
$navExtended = true;
...
if($_SESSION['CODIGO_TRABAJADOR']!=""){
        ...
        <div class="modal-content">
            ...
            <form id="formCompletarDev" style="display: none">
                ...
                <div class="row">
                    ...
                    <div class="col s6 input-field ">
                        <input type="number" id="nroVisitasDev" name="nroVisitasDev">
    ...
    <script>
        /*$.ajax({
            url: "http://app.apci.gob.pe/ApiPide/token",
            method: "POST",
            data: {
        ...
        $(document).ready(function() {
            ...
            $("#btnFinalizarEntrega").on('click',function (e) {
                e.preventDefault();
                if ($('#nroOrden').val() === ''){
                    $.alert('¡Falta agregar el Nro de ordern!');
                    return false;
                } else if ($('#documentoArchivoEntrega div.row div.col input[type="checkbox"]:checked').length !== 1){
            ...
            btnDevolver.on('click', function (e) {
                ...
                let instance = M.Modal.init(elem, {dismissible:false});
                //ContenidosTipo("codTipoMensajeriaDevolucion","16");
            ...
            $("#btnRegistrarDevolucion").on('click',function (e) {
                e.preventDefault();
                if ($('#nroDevolucion').val() === ''){
                    $.alert('¡Falta agregar el Nro de devolución!');
                    return false;
                }
                ...
                $.ajax({
                    ...
                    success : function(data) {
                        ...
                        //$('#formDevolucion').css('display', 'none');
                        //$('#btnRegistrarDevolucion').css('display', 'none');
                        //$('#btnCancelarDevolucion').css('display', 'none');
                        //$('#btnFinalizarDevolución').css('display', 'inline-block');

                        //$("#IdDespachoDevolucion").val(data.ID_DESPACHO_DEVOLUCION);
        ...
        $("div#dropzoneEntrega").dropzone({
            ...
            init: function () {
                ...
                this.on("sending", function(file, xhr, formData) {
                    formData.append('nombreCarpeta','Despacho/Entrega')
                });
                ...
                this.on("success", function(file, response) {
                    ...
                    $.each(json, function (i,fila) {
                        if (fila.evento === 'REGISTRADO' || fila.evento === 'REPETIDO'){
                            let check = '<p class="'+fila.codigo+'"><label><input type="checkbox" class="filled-in '+fila.codigo+'" checked="checked" name="documentoEntrega[]" value="'+fila.codigo+'"><span><a href="'+fila.servidor+fila.ruta+'" target="_blank">'+fila.nombre+'</a></span></label></p>';
        ...
        $("div#dropzoneDevolucion").dropzone({
            ...
            init: function () {
                ...
                this.on("sending", function(file, xhr, formData) {
                    formData.append('nombreCarpeta','Despacho/Devolucion')
                });
                ...
                this.on("success", function(file, response) {
                    ...
                    $.each(json, function (i,fila) {
                        if (fila.evento === 'REGISTRADO' || fila.evento === 'REPETIDO'){
                            let check = '<p class="'+fila.codigo+'"><label><input type="checkbox" class="filled-in '+fila.codigo+'" checked="checked" name="documentoDevolucion[]" value="'+fila.codigo+'"><span><a href="'+fila.servidor+fila.ruta+'" target="_blank">'+fila.nombre+'</a></span></label></p>';
        ...
        $("div#dropzoneCargo").dropzone({
            ...
            init: function () {
                ...
                this.on("sending", function(file, xhr, formData) {
                    formData.append('nombreCarpeta','Despacho/Cargo')
                });
                ...
                this.on("success", function(file, response) {
                    ...
                    $.each(json, function (i,fila) {
                        if (fila.evento === 'REGISTRADO' || fila.evento === 'REPETIDO'){
                            let check = '<p class="'+fila.codigo+'"><label><input type="checkbox" class="filled-in '+fila.codigo+'" checked="checked" name="documentoCargo[]" value="'+fila.codigo+'"><span><a href="'+fila.servidor+fila.ruta+'" target="_blank">'+fila.nombre+'</a></span></label></p>';
        ...
        $("#btnBuscarDNI").on("click", function (e) {
            ...
            $.ajax({
                ...
                //data: {NroDNI: $("#nroDNI").val()},
                datatype: "application/json",
                success: function (response) {
                    //let aa = JSON.parse(response.EntityResult);
                    console.log(response);
                    /*var persona = response['datosPersonaField'];
                    if (persona !== null){
                        var nombres = persona.prenombresField;
                        var apePat = persona.apPrimerField;
                        var apeMat = persona.apSegundoField;
                        var nombreCompleto = nombres + ' ' + apePat + ' ' + apeMat;
                        //$("#nombreResponsableExterno").val(datos.nombres).next().addClass('active');;
        ...
        $("#btnBuscarDNIDevolucion").on("click", function (e) {
            e.preventDefault();
            deleteSpinner();
            $("#nombreResponsableExternoDevolucion").val("");
            $.ajax({
                cache: false,
