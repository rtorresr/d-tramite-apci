...
<div class="AreaTitulo">Consulta >> Doc. Observados</div>
...
if($orden=="DESC") $cambio="ASC";
   ...
   if( $fecfinx!=''){
	...
	$sql.= " USP_CONSULTA_DOC_OBS_APROBAR '$fecinix', '$fecfinx',  '$SIx', '$NOx',  '%$cCodificacionx%', '%$cAsuntox%',  '%$cObservacionesx%', '$cCodTipoDocx' ,'$iCodOficinax','$OfiLog', '$campox', '$ordenx' ";
...
if($_GET['cCodificacion']=="" && $fecinix=="" && $fecfinx=="" && $_GET['cCodTipoDoc']=="" && $_GET['cAsunto']=="" &&  $_GET['iCodOficina']=="" && $_GET[SI]=="" && $_GET[NO]=="" ){
   $sqlin= " USP_CONSULTA_DOC_OBS_APROBAR_LISTA '$OfiLog' ";
...
*/?>    
    <td valign="top">
        ...
        ?>
        <a href="registroInternoDocumento_pdf.php?iCodTramite=<?php echo $RsTramitePDF->iCodTramite;?>" target="_blank" 
        ...
        <?php 
      		...
      		if (sqlsrv_has_rows($rsDw) > 0){
    			...
    			$sqlRep = "SELECT fFecRecepcion FROM Tra_M_Tramite_Movimientos 
					...
					?>
						<a href="registroOficinaEdit.php?iCodTramite=<?=$Rs[iCodTramite]?>&URI=<?=$_SERVER['REQUEST_URI']?>"><i class="fas fa-edit"></i>
						</a>
						
						<a href="registroObsAntesAprobar.php?iCodTramite=<?=$Rs[iCodTramite]?>&pag=2" rel="lyteframe" title="Modificar Observacion" rev="width: 410px; height: 280px; scrolling: no; border:no">
							<img src="images/icon_avance.png" width="16" height="16" border="0">
						</a>
    </td>
	</tr>
