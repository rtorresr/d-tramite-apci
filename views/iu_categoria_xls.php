...
session_start();
/**************************************************************************************
NOMBRE DEL PROGRAMA: iu_categoria_xls.php
SISTEMA: SISTEMA   DE TR�MITE DOCUMENTARIO DIGITAL
OBJETIVO: Reporte general en EXCEL  de la Tabla Categorias
...
?>
     ...
     <tbody>
<?	
    $sql=" SP_CATEGORIA_LISTA '%$_GET[cCategoria]%' ,'".$_GET['orden']."' , '".$_GET['campo']."' ";
    $rs=sqlsrv_query($cnx,$sql);

    while ($Rs=sqlsrv_fetch_array($rs)){
?>
	 <tr>
        <td colspan="3" style="width:780px;text-align:left;border: solid 1px #6F6F6F;font-size:10px;"><?=$Rs[cDesCategoria]?></td>
	 </tr>
<?php }?>
	  </tbody>
    </table>  	