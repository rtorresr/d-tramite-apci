    ...
    $RsTramitePDF=sqlsrv_fetch_object($tramitePDF);
        ...
        $rsJefe=sqlsrv_query($cnx,"SELECT * FROM Tra_M_Trabajadores WHERE iCodTrabajador='$RsTramitePDF->iCodTrabajadorRegistro'");
	        ...
	        if(sqlsrv_has_rows($rsM1)>0){
	            ...
	            $movFecha=$RsM1->fFecDerivar->format("d-m-Y h:i:s");
	        }else{
	        	$movFecha='';
	        }
            ...
            // Falla < ----------------------------------
	         $_REQUEST['data']=$_SERVER['HTTP_HOST'].'/sitdd/views/pdf_digital.php?iCodTramite='.$RsTramitePDF->iCodTramite;
        ...
        $img_logo= "<img style='width:300px' src='../images/1.jpg' alt='Logo'>";
...
// CONSULTA NUEVA
$query = "SELECT TOP 1 *,
(SELECT cNombre FROM Tra_M_Remitente WHERE iCodRemitente='$iCodRemitente') AS institucion,
...
(SELECT (RTRIM(cNombresTrabajador)+ ', ' +RTRIM(cApellidosTrabajador)) AS cNombresTrabajador FROM [dbo].[Tra_M_Trabajadores] 
	WHERE iCodTrabajador=m.iCodTrabajadorDerivar) AS para,
(SELECT cNomOficina FROM [dbo].[Tra_M_Oficinas] WHERE iCodOficina=m.iCodOficinaOrigen) AS deOficina,
(SELECT (RTRIM(cNombresTrabajador)+ ', ' +RTRIM(cApellidosTrabajador)) AS cNombresTrabajador FROM [dbo].[Tra_M_Trabajadores] 
WHERE iCodTrabajador=m.iCodTrabajadorRegistro) AS de
...
$movFecha		= $RsAprobar->FECHA_DOCUMENTO;
        ...
        <page pageset="new" backtop="10mm" backbottom="10mm" backleft="10mm" backright="10mm" footer="page">
            <page_header><!-- Cabecera de pagina --> </page_header>
                        ...
                        <tr><td colspan="4"><br></td></tr>
					        ...
					        <tr>
					          <td style="width:20%">Institucion</td><td style="width:80%">: '.$instituto.'</td>
					        </tr>
					        <tr>
					          <td style="width:20%">Destinatario</td>
					          <td style="width:80%">: '.$destinatario.'</td>
					        </tr>
    ...
    */
	...
	// conversion HTML => PDF
    $nombre=trim($RsTipDoc['cDescTipoDoc'])."-".trim($RsTramitePDF->cCodificacion).".pdf";
    print_r($nombre);
