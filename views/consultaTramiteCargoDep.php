<?php
  ...
  if ($_REQUEST[fHasta] != ''){
	...
	echo "<H3>ESTADISTICA DE ENVIO A NIVEL NACIONAL</H3>";
	echo " ";
        ...
        <?  }  ?>                      
          <th style="width: 10%; text-align: center; border: solid 1px #6F6F6F; background: #D8D8D8">DEPART.</th>			
			...
			<tbody>
				...
				?>
			  <tr>
        	<td style="width:8%;text-align:left;border: solid 1px #6F6F6F;font-size:10px;"><?php echo $RsDep[cNomDepartamento];?></td>
          <? $sqlOfis2 = " SELECT iCodOficina,cSiglaOficina FROM Tra_M_Oficinas WHERE iFlgEstado = 1 ORDER BY cSiglaOficina ASC";	$rsOfis2  = sqlsrv_query($cnx,$sqlOfis2);
          ...
          <td style="width:8%;text-align:center;border: solid 1px #6F6F6F;font-size:10px;">           
						<?
						  ...
						  $sqlContar = "SELECT dbo.fn_Rep_Cargo($RsOfis2['iCodOficina'],'$RsDep[cCodDepartamento]','$fDesde','$fHasta') ";
	   					$rsContar  = sqlsrv_query($cnx,$sqlContar);
