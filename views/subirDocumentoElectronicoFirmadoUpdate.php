...
if($_FILES['fileUpLoadDigital']['name']!=""){
	...
	if($extension[$num]=="exe" OR $extension[$num]=="dll" OR $extension[$num]=="EXE" OR $extension[$num]=="DLL"){
		...
		$nuevo_nombre = str_replace(" ","-",trim($RsTipDoc->cDescTipoDoc))."-".str_replace("/","-",$RsTramite->cCodificacion).".".$extension[$num];
		move_uploaded_file($_FILES['fileUpLoadDigital']['tmp_name'], "$rutaUpload$nuevo_nombre");
						
		$sqlDigt="UPDATE INTO Tra_M_Tramite_Digitales (iCodTramite, cNombreOriginal, cNombreNuevo) VALUES ('$RsTramite->iCodTramite', '$cNombreOriginal', '$nuevo_nombre')";
