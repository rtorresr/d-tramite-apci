...
<div class="AreaTitulo">Actualizacion - Registro de entrada con tupa</div>
		...
		<td class="FondoFormRegistro">
          ...
          sqlsrv_free_stmt($rsTipo);
			...
			<td valign="top" colspan="3"><input type="text" name="cNroDocumento" style="text-transform:uppercase" value="<?php if($_GET[clear]==""){ echo trim($Rs['cNroDocumento']); }Else{ echo $_POST['cNroDocumento'];}?>" class="FormPropertReg form-control" style="width:250px" />&nbsp;<span class="FormCellRequisito">*</span></td>
			</tr>
          ...
          $RsRmt=sqlsrv_fetch_array($rsRmt);
			...
			<td valign="top">
					<textarea name="cAsunto" style="width:340px;height:55px" class="FormPropertReg form-control"><?php if($_GET[clear]==""){ echo trim($Rs['cAsunto']); }Else{ echo $_POST['cAsunto'];}?></textarea>
					&nbsp;&nbsp;
			</td>
          ...
          sqlsrv_free_stmt($rsTupaReq);
			...
			<td valign="top" colspan="3"><input style="text-transform:uppercase" type="text" name="cReferencia" value="<?php if($_GET[clear]==""){ echo trim($Rs[cReferencia]); }Else{ echo $_POST[cReferencia];}?>" class="FormPropertReg form-control" style="width:250px" /></td>
			</tr>
					<?
          ...
          $RsTupDat=sqlsrv_fetch_array($rsTupDat);
			...
			<td>
							<?
							$sqlMov="SELECT TOP 1 * FROM Tra_M_Tramite_Movimientos WHERE iCodTramite='$_GET[iCodTramite]' And cFlgTipoMovimiento =1 ORDER BY iCodMovimiento ASC";
			...
			<td>
							...
							<?
							$sqlTrb="SELECT * FROM Tra_M_Trabajadores WHERE iCodOficina='$RsMov[iCodOficinaDerivar]' And (iCodPerfil=3 or iCodPerfil=5 ) ORDER BY iCodCategoria DESC, cNombresTrabajador ASC";
			...
			<tr>
          	...
          	if(sqlsrv_has_rows($rsDig)>0){
				...
				<td valign="top" class="CellFormRegOnly"><input type="text" name="nTiempoRespuesta" readonly value="<?=$RsTupDat[nDias]?>" class="FormPropertReg form-control" style="width:40px;text-align:right"  maxlength="3" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;"/> d�as</td>
			</tr>
