...
<ul class="collapsible">
    <li class="active">
        <div class="collapsible-header">Flujo de Trabajo Real</div>
        <div class="collapsible-body" style="display:block">
            <?php
                $sqlF = "SELECT * FROM Tra_M_Tramite_Movimientos 
                    WHERE (iCodTramite = '".$RsMovP['iCodTramite']."' OR iCodTramiteRel = '".$RsMovP['iCodTramite']."') AND (cFlgTipoMovimiento = 1 OR cFlgTipoMovimiento = 3) 
                ...
                while ($RsF = sqlsrv_fetch_array($rsF)){ ?>
                    ...
                    ?>
                    <button 
                        class="FlujoSquareEmpty btn-flow btn btn-flat tooltipped" 
                        data-position="bottom" 
                        data-tooltip="<table class>
                        <thead>
                        <tr>
                        <th>Creado por</th>
                        <th>Derivación</th>
                        ...
                        <tbody>
                        <tr>
                        <td><?php echo $RsOfiO['cNomOficina'];?></td>
                        <td><?php echo (isset($RsF['fFecMovimiento'])?$RsF['fFecMovimiento']->format("d-m-Y"):'-') .' '. (isset($RsF['fFecMovimiento'])?$RsF['fFecMovimiento']->format("h:iA"):'-'); ?></td>
                        <?php
                                if($RsF['fFecRecepcion'] === ""){
                                    echo "<td>Sin aceptar</td>";
                                }else{
                                    echo "<td>" . (isset($RsF['fFecRecepcion'])?$RsF['fFecRecepcion']->format("d-m-Y"):'-') .' '. (isset($RsF['fFecRecepcion'])?$RsF['fFecRecepcion']->format("h:iA"):'-') . "</td>";
                                }
                            ?>
                        </tr>
                        </tbody>
                    ...
                    <?php
                        ...
                        $contador++;
                    ?>   
                    <button class="FlujoSquareEmpty btn-flow tooltipped btn btn-flat" data-position="bottom" data-tooltip="<table class>
                    <thead>
                        <tr>
                        <th>Creado por</th>
                        <th>Derivación</th>
                    ...
                    <tbody>
                    <tr>
                    <td><?php echo $RsOfiD['cNomOficina']; ?></td>
                    <td><?php echo (isset($Rs['fFecMovimiento'])?$Rs['fFecMovimiento']->format("d-m-Y"):'-') .' '. (isset($Rs['fFecMovimiento'])?$RsF['fFecMovimiento']->format("h:iA"):'-'); ?></td>
                    <?php
                        if($RsF['fFecRecepcion'] === ""){
                            echo "<td>Sin aceptar</td>";
                        }else{
                            echo "<td>".(isset($Rs['fFecRecepcion'])?$RsF['fFecRecepcion']->format("d-m-Y"):'-'). " " .(isset($Rs['fFecRecepcion'])?$RsF['fFecRecepcion']->format("h:iA"):'-')."</td>";
                        }
                    ?>
                    </tr>
                    </tbody>                
                    <?php /*<dt>Delegado:</dt>
                        <?php
                            if($RsF['iCodTrabajadorDelegado'] !== ""){
                                ...
                                echo "<dd>". $RsDelg["cApellidosTrabajador"]." ".$RsDelg["cNombresTrabajador"]."</dd>";
                                sqlsrv_free_stmt($rsDelg);
                            }
                        ?> */?>
                    </table>">
                        <div class="<?php if($RsF['cFlgTipoMovimiento'] === 1) echo "FlujoSquareData"?><?php if($RsF['cFlgTipoMovimiento'] === 3) echo "FlujoSquareAnexo"?>">
                            <?php echo $RsOfiD['cSiglaOficina']?>
                        </div>
                    </button>
    ...
    <li class="">
        <div class="collapsible-header">Seguimiento de trámite</div>
                    ...
                    while ($RsM=sqlsrv_fetch_array($rsM)){ ?>
                        <tr>
                            ...
                            <td>
                                <?php
                                if($contaMov==0){
                                    echo utf8_encode(ucfirst(strtolower($Rs['cAsunto'])));
                                }else{
                                    echo utf8_encode(ucfirst(strtolower($RsM['cAsuntoDerivar'])));
                                }
                                ?>
                            </td>
                            <td>
                                <?php
                                if($contaMov==0){
                                    echo utf8_encode(ucfirst(strtolower($Rs['cObservaciones'])));
                                }Else{
                                    echo utf8_encode(ucfirst(strtolower($RsM['cObservacionesDerivar'])));
                                }
                                ?>
                            </td>
                            <td> 
    ...
    <?php if($Rs['iCodTupa']!=""){ ?>
    <li class="">
        <div class="collapsible-header">Flujo de Trabajo Programado</div>
        <div class="collapsible-body">
            <?php
                ...
                while ($RsF=sqlsrv_fetch_array($rsF)){
                    ...
                    ?>
                    <style type="text/css">
                        #area<?php echo $RsF['iCodMovFlujo']?>{
                            position: absolute;
                            display:none;
                            font-family:Arial;
                            font-size:0.8em;
                    ...
                    <script type="text/javascript">
                        function showdiv<?php echo $RsF['iCodMovFlujo']?>(event){
                            margin=8;
                            ...
                            document.getElementById('area<?php echo $RsF['iCodMovFlujo']?>').style.top = (tempY+margin);
                            document.getElementById('area<?php echo $RsF['iCodMovFlujo']?>').style.left = (tempX+margin);
                            document.getElementById('area<?php echo $RsF['iCodMovFlujo']?>').style.display='block';
                        }
                    </script>
                    <button class="FlujoSquareEmpty btn-flow btn btn-flat" onMouseOver="this.style.cursor='hand';showdiv<?php echo $RsF['iCodMovFlujo']?>(event);" onmousemove="showdiv<?php echo $RsF['iCodMovFlujo']?>(event);" onmouseout="javascript:document.getElementById('area<?php echo $RsF['iCodMovFlujo']?>').style.display='none';">
                        <div class="FlujoSquareTupa">
                            <?php echo $RsOfiO['cSiglaOficina']?>
                        </div>
                        <?php if($contador2!=sqlsrv_has_rows($rsF)){?>
                            <i class="fas fa-angle-right">
                        <?php }?>
                    </button>
                    <div id="area<?php echo $RsF['iCodMovFlujo']?>">
                        <div align=center><?php echo utf8_encode(ucwords(strtolower($RsOfiO['cNomOficina'])))?></div>
                        Actividad:
                        <?php
