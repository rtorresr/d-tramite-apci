...
function filtroString($titulo){
   // $titulo	=	ereg_replace('[^ A-Z�,./-���,./-�a�,./-z0-9_�,./(,./),./-]','',$titulo);
   $titulo	= 	html_entity_decode($titulo, ENT_QUOTES);
   $titulo = str_replace(array("&LT;", "&GT;", '&amp;', '&#039;', '&quot;','&lt;', '&gt;'), array("", "",'&','\'','"','<','>'), htmlspecialchars_decode($titulo, ENT_NOQUOTES)); 
	return($titulo);
...
<page backtop="15mm" backbottom="10mm" backleft="10mm" backright="10mm">
  ...
  <td style="width: 100%; text-align: center; border: solid 1px #585858;">
  		...
  		<td style="width:65%;text-align:left; border: solid 0px #585858;font-size:14px;font-family:Times"><b><?php echo " ".$Rs[cCodificacion]; if($Rs[cCodConcurso]!=""){ echo "  - ".$Rs[cCodConcurso];}?></b></td>
  		</tr>
  		...
  		<td style="width:65%;text-align:left; border: solid 0px #585858;font-size:14px;font-family:Times"><b>
			...
			*/
			echo date("d-m-Y G:i:s", strtotime($Rs['fFecRegistro']));
  			?>
  			</b>
  		</td>
  		</tr>
