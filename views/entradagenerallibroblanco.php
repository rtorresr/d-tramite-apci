...
<div class="AreaTitulo">Consulta >> Libro Blanco / Documento de Entrada</div>



							<form name="frmConsultaEntrada" method="GET" action="entradagenerallibroblanco.php">
						<tr>
							<td width="110" >N&ordm; Tr&aacute;mite:</td>
							<td width="390" align="left">
								<input type="txt" name="cCodificacion" value="<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>" size="28" class="FormPropertReg form-control"></td>
						...
						<tr>
							...
							<td height="10" align="left"><input type="txt" name="cNombre" value="<?=(isset($_GET['cNombre'])?$_GET['cNombre']:'')?>" size="50" class="FormPropertReg form-control"></td>
                  ...
                  ?>
                          ...
                          <tr>
                            <td >Proc. Tupa:</td>
                            <td align="left" class="CellFormRegOnly"><select name="iCodTupa" class="FormPropertReg form-control" style="width:360px" />
                            <option value="">Seleccione:</option>
                              <? 
...
<table width='100%' border="0">
   <tr>
    ...
    <tr>
        ...
        <td><img src="images/letra-l.png"></td>
        <td>Libro Blanco</td>
    </tr>
    <tr>

        <td  width='2%'><img src="images/letra-l_off.png"></td>
        <td>Libro Blanco Enviado</td>
    </tr>
</table>
...
<tr>
	<td width="98" class="headCellColum"><a href="<?=$_SERVER['PHP_SELF']?>?campo=Codificacion&orden=<?=$cambio?>&cCodificacion=<?=(isset($_GET['cCodificacion'])?$_GET['cCodificacion']:'')?>"  style=" text-decoration:<?php if($campo=="Codificacion"){ echo "underline"; }Else{ echo "none";}?>">N&ordm; Tr&aacute;mite</a></td>
	...
	<td width="220" class="headCellColum">Remitente</td> 
	...
	<td width="100" class="headCellColum">Indice</td>
	<td width="100" class="headCellColum">Oficina</td>
	
	<td class="headCellColum">Opcion</td>
	</tr>
<?
...
?>
 ...
 <tr bgcolor="<?=$color?>" onMouseOver="this.style.backgroundColor='#BFDEFF'" OnMouseOut="this.style.backgroundColor='<?=$color?>'" >
    ...
    <?php if($Rs[nFlgTipoDoc]!=4){?>
			<a href="registrodetalleprestamo.php?iCodTramite=<?=$Rs[iCodTramite]?>" rel="lyteframe" title="Detalle del Trámite" 
				 rev="width: 970px; height: 550px; scrolling: auto; border:no"><?=$Rs[cCodificacion]?>
			</a>
    <?php } else{?>
    	<a href="registrodetalleprestamo.php?iCodTramite=<?=$Rs[Relacionado]?>" rel="lyteframe" title="Detalle del Trámite" 
    		 rev="width: 970px; height: 550px; scrolling: auto; border:no"><?=$Rs[cCodificacion]?>
    	</a>
    <?php}?>
    <?
    	$sqlTra = "SELECT cApellidosTrabajador, cNombresTrabajador, ES_EXTERNO FROM Tra_M_Trabajadores 
			...
			if ($RsTra['ES_EXTERNO'] == 1) {
				echo "<div style=color:#FF00FF;>Perfil Web</div>";
			}
    ?>
   </td>
   ...
   <?php
        // indice
        $sql_ind="select (select (ltrim(rtrim(codigo))+' - '+seccion) as nombre from T_MAE_LIBRO_BLANCO where cod_auto=t.indice) as indicex from T_MOV_LIBRO_BLANCO t where cod_documento='".$Rs[iCodTramite]."'";
        $query_ind=sqlsrv_query($cnx,$sql_ind);
        $rs_ind=sqlsrv_fetch_array($query_ind);
        do{
            echo $rs_ind['indicex'];
        }while($rs_ind=sqlsrv_fetch_array($query_ind));
    ?>
    ...
    <td>
        <?php
            // indice
            $sql_of="select (select cNomOficina from Tra_M_Oficinas where iCodOficina=t.cod_oficina) as cod_oficina from T_MOV_LIBRO_BLANCO t where cod_documento='".$Rs[iCodTramite]."'";
            $query_of=sqlsrv_query($cnx,$sql_of);
            $rs_of=sqlsrv_fetch_array($query_of);
            do{
                echo $rs_of['cod_oficina'];
            }while($rs_of=sqlsrv_fetch_array($query_of));
        ?>
    </td>
    		...
    		<?php
    		    $sql="select * from T_MOV_LIBRO_BLANCO where cod_documento='".$Rs[iCodTramite]."'";
    		    ...
    		    if(sqlsrv_fetch_array($query)>0){
                    $img2="images/letra-l_off.png";
                    $mensaje_img2="Libro Blanco Enviado";
                }else{
                    $img2="images/letra-l.png";
                    $mensaje_img2="Libro Blanco";
                }
    		?>
    		
    		<a href="libroblancolista.php?id=<?=$Rs[iCodTramite]?>" rel="lyteframe" rel="lyteframe" title="<?php echo $mensaje_img2;?>" rev="width: 650px; height: 200px; scrolling: auto; border:no">
                <img src="<?php echo $img2;?>" title="<?php echo $mensaje_img2;?>" border='0' width="16px">
			</a>
